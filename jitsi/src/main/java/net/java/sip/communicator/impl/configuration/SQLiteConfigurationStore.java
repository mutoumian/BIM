...
public class SQLiteConfigurationStore extends DatabaseConfigurationStore {
    ...
    public SQLiteConfigurationStore() {
        ...
        openHelper = new SQLiteOpenHelper(context, SQLiteConfigurationStore.class.getName() + ".db", null /* factory */, 1 /* version */) {
            public void onCreate(SQLiteDatabase db) {
                db.execSQL("CREATE TABLE " + TABLE_NAME + " (" + NAME_COLUMN_NAME + " TEXT PRIMARY KEY," + VALUE_COLUMN_NAME + " TEXT" + ");");
            }
    ...
    public Object getProperty(String name) {
        ...
        if (value == null) {
            synchronized (openHelper) {
                ...
                Cursor cursor = db.query(TABLE_NAME, new String[]{VALUE_COLUMN_NAME}, NAME_COLUMN_NAME + " = ?", new String[]{name}, null /* groupBy */, null /* having */, null /* orderBy */, "1");
    ...
    public String[] getPropertyNames() {
        ...
        synchronized (openHelper) {
            SQLiteDatabase db = openHelper.getReadableDatabase();
            Cursor cursor = db.query(TABLE_NAME, new String[]{NAME_COLUMN_NAME}, null /* selection */, null /* selectionArgs */, null /* groupBy */, null /* having */, NAME_COLUMN_NAME + " ASC");

            try {
                while (cursor.moveToNext()) propertyNames.add(cursor.getString(0));
            }
            finally {
                cursor.close();
            }
    ...
    public void removeProperty(String name) {
        ...
        synchronized (openHelper) {
            ...
            db.delete(TABLE_NAME, NAME_COLUMN_NAME + " = ?", new String[]{name});
        }
    }
    ...
    public void setNonSystemProperty(String name, Object value) {
        synchronized (openHelper) {
            ...
            if (db.replace(TABLE_NAME, null /* nullColumnHack */, initialValues) == -1)
