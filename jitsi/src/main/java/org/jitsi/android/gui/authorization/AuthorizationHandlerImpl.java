...
public class AuthorizationHandlerImpl implements AuthorizationHandler {
    ...
    public void processAuthorizationResponse(AuthorizationResponse response, Contact contact) {
        ...
        if (responseCode == AuthorizationResponse.ACCEPT) {
            msg = contact.getDisplayName() + " " + ctx.getString(R.string.service_gui_AUTHORIZATION_ACCEPTED);
        }
        else if (responseCode == AuthorizationResponse.REJECT) {
            msg = contact.getDisplayName() + " " + ctx.getString(R.string.service_gui_AUTHENTICATION_REJECTED);
        }
        ...
        DialogActivity.showConfirmDialog(ctx, ctx.getString(R.string.service_gui_AUTHORIZATION_RESPONSE), msg, null, null);
    }
