...
public class Jitsi extends MainMenuActivity {
    ...
    private void showContactsFragment(Intent intent) {
        if (!AndroidUtils.isTablet()) {
            ...
            String chatID = intent.getStringExtra(ChatSessionManager.CHAT_IDENTIFIER);

            if (chatID != null) {
                Bundle args = new Bundle();

                args.putString(ChatSessionManager.CHAT_IDENTIFIER, intent.getStringExtra(ChatSessionManager.CHAT_IDENTIFIER));

                contactListFragment.setArguments(args);
            }
        }

        getSupportFragmentManager().beginTransaction().replace(R.id.contactListFragment, contactListFragment).setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN).commit();
    }
     ...
     */
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent accountLoginIntent) {
        super.onActivityResult(requestCode, resultCode, accountLoginIntent);

        switch (requestCode) {
            case OBTAIN_CREDENTIALS:
                if (resultCode == RESULT_OK) {
                    System.err.println("ACCOUNT DATA STRING====" + accountLoginIntent.getDataString());
                }
        }
    }
}