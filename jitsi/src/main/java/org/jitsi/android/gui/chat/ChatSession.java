...
public class ChatSession implements Chat, MessageListener {
    ...
    private final List<ChatSessionListener> msgListeners = new ArrayList<ChatSessionListener>();
    ...
    public ChatSession(MetaContact metaContact) {
        ...
        currentChatTransport = metaContact.getDefaultContact(OperationSetBasicInstantMessaging.class);
        ...
        while (protoContacts.hasNext()) {
            ...
            OperationSetBasicInstantMessaging imOpSet = protoContact.getProtocolProvider().getOperationSet(OperationSetBasicInstantMessaging.class);
    ...
    public void sendMessage(final String message) {
        ...
        if (pps == null) {
            logger.error("No protocol provider returned by " + currentChatTransport);
            return;
        }

        final OperationSetBasicInstantMessaging imOpSet = pps.getOperationSet(OperationSetBasicInstantMessaging.class);
    ...
    public void addTypingListener(TypingNotificationsListener l) {
        ...
        while (protoContacts.hasNext()) {
            ...
            OperationSetTypingNotifications typingOpSet = protoContact.getProtocolProvider().getOperationSet(OperationSetTypingNotifications.class);
    ...
    public void addContactStatusListener(ContactPresenceStatusListener l) {
        ...
        while (protoContacts.hasNext()) {
            ...
            OperationSetPresence presenceOpSet = protoContact.getProtocolProvider().getOperationSet(OperationSetPresence.class);
    ...
    public Collection<ChatMessage> getHistory(boolean init) {
        ...
        if (metaHistory == null) return msgCache;

        Collection<Object> history;
        if (msgCache.size() == 0) {
            history = metaHistory.findLast(chatHistoryFilter, metaContact, HISTORY_CHUNK_SIZE);
        }
        else {
            ChatMessage oldest;
            ...
            history = metaHistory.findLastMessagesBefore(chatHistoryFilter, metaContact, oldest.getDate(), HISTORY_CHUNK_SIZE);
        }
        ...
        synchronized (cacheLock) {
            ...
            if (init) return msgCache;
            else return historyMsgs;
        }
    }
    ...
    private List<ChatMessage> mergeMsgLists(List<ChatMessage> list1, List<ChatMessage> list2, int msgLimit) {
        if (msgLimit == -1) msgLimit = Integer.MAX_VALUE;
        ...
        while (list1Idx >= 0 && output.size() < msgLimit) output.add(0, list1.get(list1Idx--));

        // Input remaining list 2 messages
        while (list2Idx >= 0 && output.size() < msgLimit) output.add(0, list2.get(list2Idx--));
    ...
    public void addMessage(String contactName, Date date, String messageType, String message, String contentType) {
        ...
        ChatMessageImpl chatMsg = new ChatMessageImpl(contactName, date, chatMsgType, message, contentType);
    ...
    public String getShortDisplayName() {
        ...
        if (atIndex > -1) contactDisplayName = contactDisplayName.substring(0, atIndex);

        int spaceIndex = contactDisplayName.indexOf(" ");

        if (spaceIndex > -1) contactDisplayName = contactDisplayName.substring(0, spaceIndex);
    ...
    public void messageReceived(MessageReceivedEvent messageReceivedEvent) {
        if (!metaContact.containsContact(messageReceivedEvent.getSourceContact())) {
            return;
        }
        synchronized (cacheLock) {
    ...
    public void messageDelivered(MessageDeliveredEvent messageDeliveredEvent) {
        if (!metaContact.containsContact(messageDeliveredEvent.getDestinationContact())) {
            return;
        }
    ...
    public void messageDeliveryFailed(MessageDeliveryFailedEvent evt) {
        ...
        if (evt.getErrorCode() == MessageDeliveryFailedEvent.OFFLINE_MESSAGES_NOT_SUPPORTED) {
            errorMsg = rms.getI18NString("service.gui.MSG_DELIVERY_NOT_SUPPORTED", new String[]{sourceContact.getDisplayName()});
        }
        ...
        addMessage(metaContact.getDisplayName(), new Date(), Chat.OUTGOING_MESSAGE, sourceMessage.getContent(), sourceMessage.getContentType());

        addMessage(metaContact.getDisplayName(), new Date(), Chat.ERROR_MESSAGE, errorMsg, OperationSetBasicInstantMessaging.DEFAULT_MIME_TYPE);
    }
    ...
    private OperationSetMessageCorrection getOpSetMessageCorrection() {
        ...
        if (capOpSet != null && capOpSet.getOperationSet(currentChatTransport, OperationSetMessageCorrection.class) == null) {
            return null;
        }
    ...
    public void correctMessage(String uidToCorrect, String message) {
        try {
            ...
            mcOpSet.correctMessage(currentChatTransport, null, msg, uidToCorrect);
        }
        catch (Exception e) {
            logger.error("Message correction error for UID: " + uidToCorrect + ", new content: " + message, e);
        }
    }
    ...
    public boolean allowsTypingNotifications() {
        ProtocolProviderService protocolProviderService = currentChatTransport.getProtocolProvider();

        if (protocolProviderService == null) return false;

        Object tnOpSet = protocolProviderService.getOperationSet(OperationSetTypingNotifications.class);
    ...
    public boolean sendTypingNotification(int typingState) {
        ...
        // nothing here.
        if (!allowsTypingNotifications()) return false;
        ...
        if (protocolProvider.isRegistered() && currentChatTransport.getPresenceStatus().getStatus() >= PresenceStatus.ONLINE_THRESHOLD) {
            try {
                tnOperationSet.sendTypingNotification(currentChatTransport, typingState);
                return true;
            }
            catch (Exception ex) {
