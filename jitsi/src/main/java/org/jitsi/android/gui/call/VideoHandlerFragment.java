...
public class VideoHandlerFragment extends OSGiFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        remoteVideoContainer = (ViewGroup) activity.findViewById(R.id.remoteVideoContainer);

        localPreviewContainer = (ViewGroup) activity.findViewById(R.id.localPreviewContainer);

        callInfoGroup = (ViewGroup) activity.findViewById(R.id.callInfoGroup);

        ctrlButtonsGroup = activity.findViewById(R.id.buttonContainer);
        ...
        activity.findViewById(R.id.callVideoButton).setOnClickListener(new View.OnClickListener() {
                    @Override
        ...
        this.previewSurfaceHandler = new PreviewSurfaceProvider((OSGiActivity) activity, localPreviewContainer, true);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        int otherFacing = currentCamera.getCameraFacing() == AndroidCamera.FACING_BACK ? AndroidCamera.FACING_FRONT : AndroidCamera.FACING_BACK;
    ...
    private void updateMenu() {
        ...
        String displayName = isFrontCamera ? getString(R.string.service_gui_settings_USE_BACK_CAMERA) : getString(R.string.service_gui_settings_USE_FRONT_CAMERA);
    ...
    public void onLocalVideoButtonClicked(View callVideoButton) {
        ...
        if (isEnable) callVideoButton.setBackgroundColor(0x50000000);
        else callVideoButton.setBackgroundColor(android.graphics.Color.TRANSPARENT);
    }
    ...
    public void handleVideoEvent(CallPeer callPeer, final VideoEvent event) {
        ...
        if (event.getOrigin() == VideoEvent.REMOTE) {
            Component visualComponent = (event.getType() == VideoEvent.VIDEO_ADDED || event instanceof SizeChangeVideoEvent) ? event.getVisualComponent() : null;

            SizeChangeVideoEvent scve = event instanceof SizeChangeVideoEvent ? (SizeChangeVideoEvent) event : null;
    ...
    private void handleRemoteVideoEvent(final Component visualComponent, final SizeChangeVideoEvent scve) {
        ...
        runOnUiThread(new Runnable() {
            public void run() {
                if (getActivity() == null) {
                    logger.warn("Remote video event when getActivity() returned null");
                    return;
                }

                final View view = remoteVideoAccessor != null ? remoteVideoAccessor.getView(getActivity()) : null;

                Dimension preferredSize = selectRemotePreferredSize(visualComponent, view, scve);
                logger.info("Remote video size " + preferredSize.getWidth() + " : " + preferredSize.getHeight());
     ...
     */
    private Dimension selectRemotePreferredSize(Component visualComponent, View remoteVideoView, SizeChangeVideoEvent scve) {
        int width = -1;
        int height = -1;
    ...
    void updateCallInfoMargin() {
        ...
        if (remoteVideoContainer.getChildCount() > 0) {
            ...
            if (marginBottom < ctrlButtonsHeight && ctrlButtonsGroup.getVisibility() == View.VISIBLE) {
