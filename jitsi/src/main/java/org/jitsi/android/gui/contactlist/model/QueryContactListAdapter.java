...
public class QueryContactListAdapter extends BaseContactListAdapter implements UIGroupRenderer, ContactQueryListener {
    ...
    private List<ContactSourceService> getSources() {
        ServiceReference<?>[] sources = ServiceUtils.getServiceReferences(AndroidGUIActivator.bundleContext, ContactSourceService.class);

        List<ContactSourceService> list = new ArrayList<ContactSourceService>(sources.length);
        for (ServiceReference ref : sources) {
            ContactSourceService css = (ContactSourceService) AndroidGUIActivator.bundleContext.getService(ref);
    ...
    public Object getChild(int groupPosition, int childPosition) {
        ...
        if (groupPosition < metaGroupCount) {
            ...
            return results.get(groupPosition - groupPosition).contacts.get(childPosition);
        }
    }
