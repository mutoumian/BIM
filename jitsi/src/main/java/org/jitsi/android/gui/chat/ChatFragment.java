...
public class ChatFragment extends OSGiFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View content = inflater.inflate(R.layout.chat_conversation, container, false);
        ...
        if (chatId == null) throw new IllegalArgumentException();
    ...
    public static ChatFragment newInstance(String chatId) {
        if (logger.isDebugEnabled()) logger.debug("CHAT FRAGMENT NEW INSTANCE: " + chatId);
    ...
    public void onDetach() {
        if (logger.isDebugEnabled()) logger.debug("DETACH CHAT FRAGMENT: " + this);
    ...
    class ChatListAdapter extends BaseAdapter implements ChatSession.ChatSessionListener, ContactPresenceStatusListener, TypingNotificationsListener {
        /**
        ...
        private int getLastMessageIdx(ChatMessage newMessage) {
            ...
            if (newMessage.getCorrectedMessageUID() == null) return chatListAdapter.getCount() - 1;
        ...
        public Object getItem(int position) {
            if (logger.isDebugEnabled()) logger.debug("OBTAIN CHAT ITEM ON POSITION: " + position);
        ...
        public int getItemViewType(int position) {
            ...
            if (messageType == ChatMessage.INCOMING_MESSAGE) return INCOMING_MESSAGE_VIEW;
            ...
            else if (messageType == ChatMessage.SYSTEM_MESSAGE) return SYSTEM_MESSAGE_VIEW;
            else if (messageType == ChatMessage.ERROR_MESSAGE) return ERROR_MESSAGE_VIEW;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                if (messageViewHolder.viewType == CORRECTED_MESSAGE_VIEW || messageViewHolder.viewType == OUTGOING_MESSAGE_VIEW) {
                    int currentViewType = getItemViewType(position);
                    // Convert if has changed
                    if (messageViewHolder.viewType != currentViewType) {
                        messageViewHolder = new MessageViewHolder();

                        convertView = inflateViewForType(currentViewType, messageViewHolder, parent);
                    }
                }
            }
        ...
        private View inflateViewForType(int viewType, MessageViewHolder messageViewHolder, ViewGroup parent) {
            ...
            if (viewType == INCOMING_MESSAGE_VIEW) {
                ...
                messageViewHolder.typingView = (ImageView) convertView.findViewById(R.id.typingImageView);
            }
                ...
                convertView = inflater.inflate(viewType == SYSTEM_MESSAGE_VIEW ? R.layout.chat_system_row : R.layout.chat_error_row, parent, false);
        ...
        private void updateStatusAndAvatarView(MessageViewHolder viewHolder) {
            ...
            if (viewHolder.viewType == INCOMING_MESSAGE_VIEW) {
                avatar = MetaContactRenderer.getAvatarDrawable(chatSession.getMetaContact());

                status = MetaContactRenderer.getStatusDrawable(chatSession.getMetaContact());
            }
            else if (viewHolder.viewType == OUTGOING_MESSAGE_VIEW || viewHolder.viewType == CORRECTED_MESSAGE_VIEW) {
        ...
        public void messageDelivered(final MessageDeliveredEvent evt) {
            ...
            if (metaContact != null && chatSession.getMetaContact().equals(metaContact)) {
                final ChatMessageImpl msg = ChatMessageImpl.getMsgForEvent(evt);

                if (logger.isTraceEnabled())
                    logger.trace("MESSAGE DELIVERED: process message to chat for contact: " + contact.getAddress() + " MESSAGE: " + msg.getMessage());
        ...
        public void messageReceived(final MessageReceivedEvent evt) {
            ...
            final MetaContact metaContact = AndroidGUIActivator.getContactListService().findMetaContactByContact(protocolContact);
        ...
        public void contactPresenceStatusChanged(ContactPresenceStatusChangeEvent evt) {
            ...
            if (!chatSession.getMetaContact().containsContact(sourceContact)) return;
        ...
        public void typingNotificationReceived(TypingNotificationEvent evt) {
            ...
            TypingNotificationHandler.handleTypingNotificationReceived(evt, ChatFragment.this);
        }
        ...
        class MessageDisplay {
            ...
            public Spanned getBody() {
                if (body == null) {
                    ...
                    if (msgType == ChatMessage.OUTGOING_MESSAGE || msgType == ChatMessage.INCOMING_MESSAGE) {
    ...
    private class UpdateStatusTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            ...
            for (int i = 0; i < chatListView.getChildCount(); i++) {
                ...
                if (viewHolder != null && viewHolder.viewType == ChatListAdapter.INCOMING_MESSAGE_VIEW) {
                    Drawable status = MetaContactRenderer.getStatusDrawable(chatSession.getMetaContact());
    ...
    public void setAvatar(ImageView avatarView, Drawable avatarDrawable) {
        if (avatarDrawable == null) {
            avatarDrawable = JitsiApplication.getAppResources().getDrawable(R.mipmap.avatar);
        }
    ...
    public void setTypingState(int typingState) {
        ...
        TextView typingTextView = (TextView) typingView.findViewById(R.id.typingTextView);
        ImageView typingImgView = (ImageView) typingView.findViewById(R.id.typingImageView);
        ...
        if (typingState == OperationSetTypingNotifications.STATE_TYPING) {
            ...
            typingTextView.setText(chatSession.getShortDisplayName() + " " + JitsiApplication.getResString(R.string.service_gui_CONTACT_TYPING));
            setVisible = true;
        }
        else if (typingState == OperationSetTypingNotifications.STATE_PAUSED) {
            typingImgView.setImageResource(R.mipmap.typing1);
            typingTextView.setText(chatSession.getShortDisplayName() + " " + JitsiApplication.getResString(R.string.service_gui_CONTACT_PAUSED_TYPING));
            setVisible = true;
        }
        ...
        else typingView.setVisibility(View.INVISIBLE);
    }
