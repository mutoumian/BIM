...
public class OtrFingerprints extends OSGiActivity {
    ...
    List<Contact> getContacts() {
        List<Contact> allContacts = new Vector<Contact>();

        // Get the protocolproviders
        ServiceReference[] protocolProviderRefs = ServiceUtils.getServiceReferences(OtrActivator.bundleContext, ProtocolProviderService.class);

        if (protocolProviderRefs.length < 1) return allContacts;

        // Get the metacontactlist service.
        MetaContactListService service = ServiceUtils.getService(OtrActivator.bundleContext, MetaContactListService.class);
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        boolean keyExists = OtrActivator.scOtrKeyManager.getRemoteFingerprint(contact) != null;
    ...
    class FingerprintListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            View row = getLayoutInflater().inflate(R.layout.otr_fingerprint_row, parent, false);

            Contact contact = (Contact) getItem(position);

            ScOtrKeyManager keyManager = OtrActivator.scOtrKeyManager;

            ViewUtil.setTextViewValue(row, R.id.accountName, contact.getDisplayName());

            ViewUtil.setTextViewValue(row, R.id.fingerprint, keyManager.getRemoteFingerprint(contact));

            int stringRes = keyManager.isVerified(contact) ? R.string.plugin_otr_configform_COLUMN_VALUE_VERIFIED_TRUE : R.string.plugin_otr_configform_COLUMN_VALUE_VERIFIED_FALSE;

            String verifyStr = getString(R.string.plugin_otr_configform_VERIFY_STATUS, getString(stringRes));
