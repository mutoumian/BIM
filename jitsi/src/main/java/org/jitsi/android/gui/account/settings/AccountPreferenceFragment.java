...
public abstract class AccountPreferenceFragment extends OSGiPreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void loadAccount(AccountID account) {
        ...
        if (wizard == null) throw new NullPointerException();
    ...
    protected void onPreferencesCreated() {
        ...
        String audioEncCatKey = getResources().getString(R.string.pref_cat_audio_encoding);
        Preference audioEncPreference = findPreference(audioEncCatKey);
        if (audioEncPreference != null) {
            audioEncPreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
        ...
        String videoEncCatKey = getResources().getString(R.string.pref_cat_video_encoding);
        Preference videoEncPreference = findPreference(videoEncCatKey);
        if (videoEncPreference != null) {
            videoEncPreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                        public boolean onPreferenceClick(Preference preference) {
        ...
        String encOnOffKey = getResources().getString(R.string.pref_key_enable_encryption);
        Preference encryptionOnOff = findPreference(encOnOffKey);
        if (encryptionOnOff != null) {
            encryptionOnOff.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                        public boolean onPreferenceClick(Preference preference) {
    ...
    AccountRegistrationWizard findRegistrationService(String protocolName) {
        ...
        try {
            ...
            for (int i = 0; i < accountWizardRefs.length; i++) {
                AccountRegistrationWizard wizard = (AccountRegistrationWizard) context.getService(accountWizardRefs[i]);

                if (wizard.getProtocolName().equals(protocolName)) return wizard;
            }
        }
    ...
    private void startSecurityActivity() {
        ...
        if (securityRegistration == null) throw new NullPointerException();
    ...
    private void startEncodingActivity(MediaType mediaType) {
        Intent intent = new Intent(getActivity(), EncodingActivity.class);
        ...
        if (encodingsRegistration == null) throw new NullPointerException();
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == EDIT_ENCODINGS && resultCode == Activity.RESULT_OK) {
            ...
            EncodingsRegistrationUtil encReg = (EncodingsRegistrationUtil) data.getSerializableExtra(EncodingActivity.EXTRA_KEY_ENC_REG);
            ...
            SecurityAccountRegistration secReg = (SecurityAccountRegistration) data.getSerializableExtra(SecurityActivity.EXTR_KEY_SEC_REGISTRATION);
