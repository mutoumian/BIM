 ...
 * @author Pawel Domas
 */
public class ChatActivity extends OSGiActivity implements OnPageChangeListener {
    /**
     * The logger
     */
    ...
    protected void onDestroy() {
        ...
        if (chatPagerAdapter != null) chatPagerAdapter.dispose();
    ...
    private void handleIntent(Intent intent, Bundle savedInstanceState) {
        ...
        if (chatId == null) throw new RuntimeException("Missing chat identifier extra");

        ChatSession session = ChatSessionManager.createChatForMetaUID(chatId);
        if (session == null) {
            logger.error("Failed to create chat session for meta UID: " + chatId);
            return;
        }
