...
public class OtrFragment extends OSGiFragment implements ChatSessionManager.CurrentChatListener {
    ...
    private final ScOtrEngineListener scOtrEngineListener = new ScOtrEngineListener() {
        ...
        public void globalPolicyChanged() {
            if (currentContact != null)
                setPolicy(OtrActivator.scOtrEngine.getContactPolicy(currentContact));
        }
    };
    ...
    public void setCurrentContact(Contact contact) {
        ...
        if (contact != null && contact.getProtocolProvider() != null) {
            msgingOpSet = contact.getProtocolProvider().getOperationSet(OperationSetBasicInstantMessaging.class);
        }
    ...
    public void setCurrentChatSession(String chatSessionKey) {
        ChatSession activeChat = ChatSessionManager.getActiveChat(chatSessionKey);
    ...
    private void setPolicy(final OtrPolicy contactPolicy) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                getPadlock().setVisible(contactPolicy != null);

                getPadlock().setEnabled(contactPolicy != null && contactPolicy.getEnableManual());
            }
        });
    ...
    private void setStatus(ScSessionStatus status) {
        ...
        switch (status) {
            case ENCRYPTED:
                iconId = OtrActivator.scOtrKeyManager.isVerified(currentContact) ? R.mipmap.encrypted_verified : R.mipmap.encrypted_unverified;
                break;
            case FINISHED:
                iconId = R.mipmap.encrypted_finished;
                break;
            case PLAINTEXT:
                iconId = R.mipmap.encrypted_unsecure;
                break;
            default:
                return;
        }
