    ...
    protected void doInitialize() throws Exception {
        ...
        for (int cameraId = 0; cameraId < Camera.getNumberOfCameras(); cameraId++) {
            ...
            if (previewSizes == null) {
                 ...
                 */
                logger.warn("getSupportedVideoSizes returned null for camera: " + cameraId);
                previewSizes = params.getSupportedPreviewSizes();
            }

            logger.info("Video sizes supported by " + locator.toString() + ": " + CameraUtils.cameraSizesToString(previewSizes));
            ...
            logger.info("Image formats supported by " + locator.toString() + ": " + CameraUtils.cameraImgFormatsToString(camFormats));
            ...
            if (AndroidEncoder.isDirectSurfaceEnabled()) {
                ...
                for (Dimension size : sizes) {
                    formats.add(new VideoFormat(Constants.ANDROID_SURFACE, size, Format.NOT_SPECIFIED, Surface.class, Format.NOT_SPECIFIED));
                }
                /*
            ...
            if (camFormats.contains(Integer.valueOf(ImageFormat.YV12))) {
                // Image formats
                for (Dimension size : sizes) {
                    formats.add(new YUVFormat(size, Format.NOT_SPECIFIED, Format.byteArray, YUVFormat.YUV_420, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED));
                }
            ...
            String name = cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT ? res.getString(R.string.service_gui_settings_FRONT_CAMERA) : res.getString(R.string.service_gui_settings_BACK_CAMERA);
            name += " (AndroidCamera)";
