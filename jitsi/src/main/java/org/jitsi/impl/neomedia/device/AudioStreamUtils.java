...
public class AudioStreamUtils {
     ...
     */
    public static InputStream getAudioInputStream(String url) throws IOException {
        ...
        try {
            ...
            if ((lastPathSeparator > -1) && ((extensionStartIx = url.lastIndexOf('.')) > lastPathSeparator))
                resourceUri = url.substring(0, extensionStartIx);
            else resourceUri = url;
    ...
    public static AudioFormat getFormat(InputStream audioInputStream) {
        ...
        return new javax.media.format.AudioFormat(javax.media.format.AudioFormat.LINEAR, waveHeader.getSampleRate(), waveHeader.getBitsPerSample(), waveHeader.getChannels());
    }
}
