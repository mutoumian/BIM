...
public class AudioTrackRenderer extends AbstractAudioRenderer<AudioSystem> {
    ...
    private static final int ABSTRACT_VOLUME_CONTROL_PERCENT_RANGE = (BasicVolumeControl.MAX_VOLUME_PERCENT - BasicVolumeControl.MIN_VOLUME_PERCENT) / 100;
    ...
    public Format[] getSupportedInputFormats() {
        if (supportedInputFormats == null) {
            ...
            supportedInputSampleRates[supportedInputSampleRateCount] = AudioTrack.getNativeOutputSampleRate(getStreamType());
            supportedInputSampleRateCount++;
            System.arraycopy(Constants.AUDIO_SAMPLE_RATES, 0, supportedInputSampleRates, supportedInputSampleRateCount, Constants.AUDIO_SAMPLE_RATES.length);
            ...
            for (int i = 0; i < supportedInputSampleRateCount; i++) {
                ...
                supportedInputFormats[2 * i] = new AudioFormat(AudioFormat.LINEAR, sampleRate, 16 /* sampleSizeInBits */, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
                supportedInputFormats[2 * i + 1] = new AudioFormat(AudioFormat.LINEAR, sampleRate, 8 /* sampleSizeInBits */, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
            }
        }
    ...
    public synchronized void open() throws ResourceUnavailableException {
        if (audioTrack == null) {
            ...
            audioTrack = new AudioTrack(getStreamType(), (int) sampleRate, channelConfig, audioFormat, Math.max(audioTrackBufferSizeInBytes, AudioTrack.getMinBufferSize((int) sampleRate, channelConfig, audioFormat)), AudioTrack.MODE_STREAM);
            ...
            if (USE_SOFTWARE_GAIN) {
                ...
                if (setStereoVolume != AudioTrack.SUCCESS) {
                    logger.warn("AudioTrack.setStereoVolume(float, float) failed" + " with return value " + setStereoVolume);
                }
            }
            else {
                /*
            ...
            if ((data == null) || (length == 0)) {
                ...
                synchronized (this) {
                    if (audioTrack == null) {
                        ...
                        if (gainControl != null) {
                            if (USE_SOFTWARE_GAIN) {
                                ...
                                if (gainControlLevelAppliedToAudioTrack != gainControlLevel) {
                                    ...
                                    float volume = minVolume + AUDIO_TRACK_VOLUME_RANGE * gainControlLevel * ABSTRACT_VOLUME_CONTROL_PERCENT_RANGE;
                                    ...
                                    if ((setStereoVolume != AudioTrack.SUCCESS) || (volume > maxVolume)) {
                                        BasicVolumeControl.applyGain(gainControl, bytes, offset, length);
                                    }
                                }
                            }
                        ...
                        if (latency == null) written = audioTrack.write(bytes, offset, length);
                        else {
                            /*
                        ...
                        if (written < 0) processed = BUFFER_PROCESSED_FAILED;
                        else {
                            processed = BUFFER_PROCESSED_OK;
                            if (written == 0) {
                                /*
    ...
    private void runInLatencyThread() {
        try {
            ...
            while (true) {
                synchronized (this) {
                    if (!Thread.currentThread().equals(latencyThread)) break;
                    if (audioTrack == null) break;

                    boolean wait = false;

                    if (!latencyIncurred) {
                        if (latencyLength < (latency.length / 2)) wait = true;
                        else latencyIncurred = true;
                    }
                    else if (latencyLength <= 0) wait = true;
                    ...
                    int toWrite = Math.min(Math.min(latencyLength, latency.length - latencyHead), 2 * audioTrackWriteLengthInBytes);
                    ...
                    if (written < 0) {
                        ...
                        latencyHead += written;
                        if (latencyHead >= latency.length) latencyHead = 0;
