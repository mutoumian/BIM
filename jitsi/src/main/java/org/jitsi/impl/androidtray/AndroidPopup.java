...
public class AndroidPopup {
    ...
    protected AndroidPopup(NotificationPopupHandler handler, PopupMessage popupMessage) {
        ...
        if (popupMessage.getGroup() == null) {
            ...
            if (AndroidNotifications.MESSAGE_GROUP.equals(group)) {
                ...
                this.smallIcon = R.mipmap.incoming_message;
            }
            else if (AndroidNotifications.CALL_GROUP.equals(group)) {
                this.smallIcon = R.mipmap.missed_call;
            }
        }
    ...
    public void removeNotification() {
        ...
        NotificationManager notifyManager = JitsiApplication.getNotificationManager();
    ...
    public boolean isChatRelated(ChatSession session) {
        return contact != null && AndroidNotifications.MESSAGE_GROUP.equals(popupMessage.getGroup()) && session.getMetaContact().containsContact(contact);
    }
    ...
    public AndroidPopup tryMerge(PopupMessage popupMessage) {
        if (this.isGroupTheSame(popupMessage) && isContactTheSame(popupMessage)) {
    ...
    NotificationCompat.Builder buildNotification() {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(ctx).setSmallIcon(smallIcon).setContentTitle(popupMessage.getMessageTitle()).setContentText(getMessage()).setAutoCancel(true)// will be cancelled once clciked
        ...
        if (AndroidUtils.hasAPI(11)) {
            ...
            if (iconBmp == null && contact != null) {
                iconBmp = AndroidImageUtil.scaledBitmapFromResource(res, R.mipmap.avatar, prefWidth, prefHeight);
            }
    ...
    public PendingIntent constructIntent() {
        ...
        if (tag instanceof Contact) {
            ...
            if (AndroidNotifications.MESSAGE_GROUP.equals(group)) {
                ...
                if (targetIntent == null) {
                    logger.error("Failed to create chat with " + metaContact);
                }
            }
            else {
        ...
        if (message != null && targetIntent == null) {
            targetIntent = DialogActivity.getDialogIntent(JitsiApplication.getGlobalContext(), message.getMessageTitle(), message.getMessage());
        }

        if (targetIntent == null) return null;

        return PendingIntent.getActivity(JitsiApplication.getGlobalContext(), getId(), /*
