...
public class AndroidContactQuery extends AbstractContactQuery<AndroidContactSource> {
     ...
     */
    private final static String DISPLAY_NAME_COLUMN = AndroidUtils.hasAPI(Build.VERSION_CODES.HONEYCOMB) ? ContactsContract.Contacts.DISPLAY_NAME_PRIMARY : ContactsContract.Contacts.DISPLAY_NAME;

    /**
     * Selection query
     */
    private static final String SELECTION = DISPLAY_NAME_COLUMN + " LIKE ?" + " AND " + ContactsContract.Contacts.HAS_PHONE_NUMBER + " > 0";

    /**
     * List of projection columns that will be returned
     */
    private static final String[] PROJECTION = {ContactsContract.Contacts._ID, ContactsContract.Contacts.LOOKUP_KEY, DISPLAY_NAME_COLUMN, ContactsContract.Contacts.PHOTO_THUMBNAIL_URI, ContactsContract.Contacts.PHOTO_URI, ContactsContract.Contacts.PHOTO_ID};
    ...
    private void doQuery() {
        ...
        Cursor rCursor = ctx.getContentResolver().query(CONTACTS_URI, PROJECTION, SELECTION, new String[]{queryString}, null);
        ...
        try {
            ...
            int LOOP_UP = rCursor.getColumnIndex(ContactsContract.Contacts.LOOKUP_KEY);
            int DISPLAY_NAME = rCursor.getColumnIndex(DISPLAY_NAME_COLUMN);
            //int HAS_PHONE = rCursor.getColumnIndex(
            //    ContactsContract.Contacts.HAS_PHONE_NUMBER);
            int THUMBNAIL_URI = rCursor.getColumnIndex(ContactsContract.Contacts.PHOTO_THUMBNAIL_URI);
            int PHOTO_URI = rCursor.getColumnIndex(ContactsContract.Contacts.PHOTO_URI);
            int PHOTO_ID = rCursor.getColumnIndex(ContactsContract.Contacts.PHOTO_ID);
            ...
            while (rCursor.moveToNext()) {
                ...
                Cursor result = null;
                try {
                    result = ctx.getContentResolver().query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, new String[]{ContactsContract.CommonDataKinds.Phone.DATA}, ContactsContract.CommonDataKinds.Phone.LOOKUP_KEY + "=?", new String[]{String.valueOf(lookUp)}, null);
                    ...
                    while (result.moveToNext() && !cancel) {
                        ...
                        results.add(new AndroidContact(getContactSource(), id, lookUp, displayName, thumbnail, photoUri, photoId, phone));
                    }
                }
                finally {
