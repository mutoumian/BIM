...
public class ChatSessionManager {
     ...
     */
    private static final Map<String, ChatSession> activeChats = new LinkedHashMap<String, ChatSession>();

    /**
     * The list of chat listeners.
     */
    private static final List<ChatListener> chatListeners = new ArrayList<ChatListener>();

    /**
     * The list of active chats.
     */
    private static final List<CurrentChatListener> currentChatListeners = new ArrayList<CurrentChatListener>();

    /**
     * The list of chat link listeners.
     */
    private static final List<ChatLinkClickedListener> chatLinkListeners = new ArrayList<ChatLinkClickedListener>();
    ...
    public synchronized static void removeAllActiveChats() {
        ArrayList<ChatSession> sessions = new ArrayList<ChatSession>(activeChats.values());
     ...
     * identifier
     */
    public synchronized static ChatSession getActiveChat(MetaContact metaContact) {
    ...
    public synchronized static void setCurrentChatId(String chatId) {
        ...
        if (currChat != null) {
            logger.debug("Current chat with: " + currChat.getMetaContact().getDisplayName());
    ...
    public synchronized static Chat findChatForContact(Contact contact, boolean startIfNotExists) {
        ...
        if (!startIfNotExists) return null;
     ...
     */
    public synchronized static ChatSession createChatForMetaUID(String metaContactUid) {
        if (metaContactUid == null) throw new NullPointerException();

        MetaContact metaContact = AndroidGUIActivator.getContactListService().findMetaContactByMetaUID(metaContactUid);
        if (metaContact == null) {
            logger.error("Meta contact not found for meta UID: " + metaContactUid);
            return null;
        }
    ...
    public static Intent getChatIntent(MetaContact contact) {
        ...
        if (AndroidUtils.isTablet()) {
            // Use home activity
            chatIntent = new Intent(JitsiApplication.getGlobalContext(), Jitsi.class);
        }
        else {
            // Use chat activity
            chatIntent = new Intent(JitsiApplication.getGlobalContext(), ChatActivity.class);
        }
    ...
    public static Intent getLastChatIntent() {
        if (lastChatContact == null) return null;
        else return getChatIntent(lastChatContact);
    }
