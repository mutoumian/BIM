...
public class MergedMessage implements ChatMessage {
    ...
    ChatMessage getMessageForCorrection() {
        if (children.size() > 0) {
            ...
            if (candidate.getUidForCorrection() != null && candidate.getContentForCorrection() != null)
                return candidate;
        }
        return root;
    }
    ...
    public boolean isConsecutiveMessage(ChatMessage nextMsg) {
        return findCorrectedMessage(nextMsg) != null || root.isConsecutiveMessage(nextMsg);
    }
}
