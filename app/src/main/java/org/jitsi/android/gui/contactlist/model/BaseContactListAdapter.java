...
public abstract class BaseContactListAdapter extends BaseExpandableListAdapter {
    ...
    public void invalidateViews() {
        if (contactListView == null) return;
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if (isExtendedChat) {
                ...
                ViewUtil.ensureVisible(convertView, R.id.rightShadowBottom, true);
            }
            ...
            contactViewHolder.callButtonLayout.setOnClickListener(callButtonListener);
        ...
        if (isShowVideoCall || isShowCall) {
            ...
            if (renderer.isSelected(child)) {
                if (isShowVideoCall)
                    contactViewHolder.callButton.setImageResource(R.drawable.video_call_selected);
                else
                    contactViewHolder.callButton.setImageResource(R.drawable.contact_call_selected);
            }
            else {
                if (isShowVideoCall)
                    contactViewHolder.callButton.setImageResource(R.drawable.video_call);
                else contactViewHolder.callButton.setImageResource(R.drawable.contact_call);
            }
        }
        else {
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        groupViewHolder.displayName.setText(groupRenderer.getDisplayName(group).toUpperCase());

        // Group expand indicator
        int indicatorResId = isExpanded ? R.drawable.expanded : R.drawable.collapsed;
    ...
    private class CallButtonClickListener implements View.OnClickListener {
        public void onClick(View view) {
            ...
            if (contact == null) {
                logger.warn("No contact to make a call at " + viewHolder.groupPosition + ", " + viewHolder.childPosition);
                return;
            }

            UIContactRenderer renderer = getContactRenderer(viewHolder.groupPosition);

            AndroidCallUtil.createAndroidCall(JitsiApplication.getGlobalContext(), viewHolder.callButton, renderer.getDefaultAddress(contact));
        }
    }
    ...
    private void setAvatar(ImageView avatarView, Drawable avatarImage) {
        if (avatarImage == null) {
            avatarImage = JitsiApplication.getAppResources().getDrawable(R.drawable.avatar);
        }
