...
public class ChatController implements AdapterView.OnItemClickListener, TextView.OnEditorActionListener, View.OnClickListener, TextWatcher {
    ...
    public void onShow() {
        ...
        msgEdit.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_SHORT_MESSAGE | InputType.TYPE_TEXT_FLAG_CAP_SENTENCES);
        ...
        if (session.allowsTypingNotifications()) msgEdit.addTextChangedListener(this);
    ...
    public void onItemClick(AdapterView<?> adapter, View view, int position, long id) {
        ...
        if (position != chatListAdapter.getCount() - 1) {
            for (int i = position + 1; i < chatListAdapter.getCount(); i++) {
                if (chatListAdapter.getMessage(i).getMessageType() == ChatMessage.OUTGOING_MESSAGE) {
                    cancelCorrection();
                    return;
                }
            }
    ...
    private void updateCorrectionState() {
        ...
        int bgColorId = correction ? R.color.msg_correction_bg : R.color.blue;
    ...
    class TypingControl extends Thread {
        ...
        public void run() {
            while (typingState != OperationSetTypingNotifications.STATE_STOPPED) {
                ...
                if (cancel) {
                    setNewTypingState(OperationSetTypingNotifications.STATE_STOPPED);
                    break;
                }
                else if (restart) {
