...
public class SummaryMapper implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    /**
     * The key to {@link Preference} mapping
     */
    private Map<String, Preference> mappedPreferences = new HashMap<String, Preference>();
     ...
     */
    private Map<String, SummaryConverter> convertersMap = new HashMap<String, SummaryConverter>();

    /**
     * Mapping containing empty string definitions
     */
    private Map<String, String> emptyStrMap = new HashMap<String, String>();
    ...
    public void includePreference(Preference pref, String empty, SummaryConverter converter) {
        if (pref == null) throw new NullPointerException("The preference can not be null");

        String key = pref.getKey();
        mappedPreferences.put(key, pref);
        emptyStrMap.put(key, empty);

        if (converter != null) convertersMap.put(pref.getKey(), converter);
