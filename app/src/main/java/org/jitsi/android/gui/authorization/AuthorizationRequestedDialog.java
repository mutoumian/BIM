...
public class AuthorizationRequestedDialog extends OSGiActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ViewUtil.setTextViewValue(content, R.id.requestInfo, getString(R.string.service_gui_AUTHORIZATION_REQUESTED_INFO, request.contact.getDisplayName()));
    ...
    protected void onDestroy() {
        ...
        if (ViewUtil.isCompoundChecked(getContentView(), R.id.addToContacts) && responseCode.equals(AuthorizationResponse.ACCEPT)) {
            // Add to contacts
            Spinner groupSpinner = (Spinner) findViewById(R.id.selectGroupSpinner);

            ContactListUtils.addContact(request.contact.getProtocolProvider(), (MetaContactGroup) groupSpinner.getSelectedItem(), request.contact.getAddress());
        }
