...
public class RemoveAccountDialog {
    public static AlertDialog create(Context ctx, final AccountID account, final OnAccountRemovedListener listener) {
        ...
        return alert.setTitle(R.string.service_gui_REMOVE_ACCOUNT).setMessage(ctx.getString(R.string.service_gui_REMOVE_ACCOUNT_MESSAGE, account.getDisplayName())).setPositiveButton(R.string.service_gui_YES, new DialogInterface.OnClickListener() {
                    @Override
    ...
    private static void onRemoveClicked(final DialogInterface dialog, final AccountID account, final OnAccountRemovedListener l) {
    ...
    private static void removeAccount(AccountID accountID) {
        ...
        if (!isUninstalled) throw new RuntimeException("Failed to uninstall account");
    }
