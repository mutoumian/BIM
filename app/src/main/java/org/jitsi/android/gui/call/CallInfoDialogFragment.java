...
public class CallInfoDialogFragment extends OSGiDialogFragment {
    ...
    private void updateIceSection(CallPeerMediaHandler<?> callPeerMediaHandler) {
        ...
        int[] harvesterLabels = {R.id.googleTurnLabel, R.id.googleTurnSSlLabel, R.id.hostHarvesterLabel, R.id.jingleNodesLabel, R.id.stunHarvesterLabel, R.id.turnHarvesterLabel, R.id.upnpHarvesterLabel};
        int[] harvesterValues = {R.id.googleTurnTime, R.id.googleTurnSSlTime, R.id.hostHarvesterTime, R.id.jingleNodesTime, R.id.stunHarvesterTime, R.id.turnHarvesterTime, R.id.upnpHarvesterTime};
    ...
    private String getHarvestTimeStr(long harvestingTime, int harvestCount) {
        ...
        harvestStr.append(" ");
        harvestStr.append(getString(R.string.service_gui_callinfo_HARVESTING_MS_FOR));
        harvestStr.append(" ");
    ...
    private String getStreamEncryptionMethod(CallPeerMediaHandler<?> callPeerMediaHandler, MediaStream mediaStream, MediaType mediaType) {
        ...
        if (srtpControl != null) {
            ...
            rtpType = resources.getString(R.string.service_gui_callinfo_MEDIA_STREAM_SRTP) + " (" + resources.getString(R.string.service_gui_callinfo_KEY_EXCHANGE_PROTOCOL) + ": " + info + ")";
        }
    ...
    private void updateAudioVideoInfo(CallPeerMediaHandler<?> callPeerMediaHandler, MediaType mediaType) {
        View container = mediaType == MediaType.AUDIO ? getView().findViewById(R.id.audioInfo) : getView().findViewById(R.id.videoInfo);
        ...
        if (mediaType == MediaType.VIDEO) {
            ...
            if (downloadVideoSize != null || uploadVideoSize != null) {
                hasVideoSize = true;
                setTextViewValue(container, R.id.videoSize, DOWN_ARROW + " " + this.videoSizeToString(downloadVideoSize) + " " + UP_ARROW + " " + this.videoSizeToString(uploadVideoSize));
            }
        }
        ...
        if (iceLocalHostVisible) {
            setTextViewValue(container, R.id.iceLocalHost, iceLocalHostAddress.getAddress().getHostAddress() + "/" + iceLocalHostAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (iceLocalReflexiveVisible) {
            setTextViewValue(container, R.id.iceLocalReflx, iceLocalReflexiveAddress.getAddress().getHostAddress() + "/" + iceLocalReflexiveAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (iceLocalRelayedAddress != null) {
            setTextViewValue(container, R.id.iceLocalRelayed, iceLocalRelayedAddress.getAddress().getHostAddress() + "/" + iceLocalRelayedAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (isIceRemoteRelayed) {
            setTextViewValue(container, R.id.iceRemoteRelayed, iceRemoteRelayedAddress.getAddress().getHostAddress() + "/" + iceRemoteRelayedAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (isIceRemoteReflexive) {
            setTextViewValue(container, R.id.iceRemoteReflexive, iceRemoteReflexiveAddress.getAddress().getHostAddress() + "/" + iceRemoteReflexiveAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (isIceRemoteHost) {
            setTextViewValue(container, R.id.iceRemoteHost, iceRemoteHostAddress.getAddress().getHostAddress() + "/" + iceRemoteHostAddress.getPort());
            displayedIpPort = true;
        }
        ...
        if (!displayedIpPort) {
            setTextViewValue(container, R.id.localIp, mediaStreamStats.getLocalIPAddress() + " / " + String.valueOf(mediaStreamStats.getLocalPort()));
            setTextViewValue(container, R.id.remoteIp, mediaStreamStats.getRemoteIPAddress() + " / " + String.valueOf(mediaStreamStats.getRemotePort()));
        }

        // Bandwidth.
        String bandwidthStr = DOWN_ARROW + " " + (int) mediaStreamStats.getDownloadRateKiloBitPerSec() + " Kbps " + " " + UP_ARROW + " " + (int) mediaStreamStats.getUploadRateKiloBitPerSec() + " Kbps";
        setTextViewValue(container, R.id.bandwidth, bandwidthStr);

        // Loss rate.
        String lossRateStr = DOWN_ARROW + " " + (int) mediaStreamStats.getDownloadPercentLoss() + "% " + UP_ARROW + " " + (int) mediaStreamStats.getUploadPercentLoss() + "%";
        ...
        // Discarded percent.
        setTextViewValue(container, R.id.discardedPercent, String.valueOf((int) mediaStreamStats.getPercentDiscarded() + "%"));

        // Discarded total.
        String discardedTotalStr = String.valueOf(mediaStreamStats.getNbDiscarded()) + " (" + mediaStreamStats.getNbDiscardedLate() + " late, " + mediaStreamStats.getNbDiscardedFull() + " full, " + mediaStreamStats.getNbDiscardedShrink() + " shrink, " + mediaStreamStats.getNbDiscardedReset() + " reset)";
        ...
        // Jitter buffer delay.
        String jitterDelayStr = "~" + mediaStreamStats.getJitterBufferDelayMs() + "ms; currently in queue: " + mediaStreamStats.getPacketQueueCountPackets() + "/" + mediaStreamStats.getPacketQueueSize() + " packets";
        ...
        // Jitter.
        setTextViewValue(container, R.id.jitter, DOWN_ARROW + " " + (int) mediaStreamStats.getDownloadJitterMs() + " ms " + UP_ARROW + (int) mediaStreamStats.getUploadJitterMs() + " ms");
    }
