...
public class AccountsListActivity extends OSGiActivity {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (id == R.id.remove) {
            RemoveAccountDialog.create(this, clickedAccount.getAccountID(), new RemoveAccountDialog.OnAccountRemovedListener() {
                        @Override
    ...
    private void startPresenceActivity(Account account) {
        Intent statusIntent = new Intent(getBaseContext(), PresenceStatusActivity.class);
        statusIntent.putExtra(PresenceStatusActivity.INTENT_ACCOUNT_ID, account.getAccountID().getAccountUniqueID());
        ...
        protected View getView(boolean isDropDown, final Account account, ViewGroup parent, LayoutInflater inflater) {
            ...
            rowView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (account.getProtocolProvider() != null) {
                        ...
                        String msg = getString(R.string.service_gui_ACCOUNT_DISCONNECTED, account.getAccountName());
            ...
            button.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                        public void onCheckedChanged(CompoundButton compoundButton, boolean enable) {
                            ...
                            String message = enable ? getString(R.string.service_gui_CONNECTING_ACCOUNT, account.getAccountName()) : getString(R.string.service_gui_DISCONNECTING_ACCOUNT, account.getAccountName());
    ...
    class AccountEnableThread extends Thread {
        ...
        public void run() {
            try {
                if (enable) accountManager.loadAccount(account);
                else accountManager.unloadAccount(account);
            }
            catch (OperationFailedException e) {
                AndroidUtils.showAlertDialog(JitsiApplication.getGlobalContext(), getString(R.string.service_gui_ERROR), "Failed to " + (enable ? "load" : "unload") + " " + account);

                logger.error(e.getMessage(), e);
            }
            finally {
                if (DialogActivity.waitForDialogOpened(progressDialog)) {
                    DialogActivity.closeDialog(JitsiApplication.getGlobalContext(), progressDialog);
                }
                else {
