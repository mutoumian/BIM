...
public class SettingsActivity extends OSGiActivity {
    ...
    // Message section
    static private final String P_KEY_LOG_CHAT_HISTORY = JitsiApplication.getResString(R.string.pref_key_history_logging);
    static private final String P_KEY_SHOW_HISTORY = JitsiApplication.getResString(R.string.pref_key_show_history);
    static private final String P_KEY_HISTORY_SIZE = JitsiApplication.getResString(R.string.pref_key_chat_history_size);
    static private final String P_KEY_TYPING_NOTIFICATIONS = JitsiApplication.getResString(R.string.pref_key_typing_notifications);
    static private final String P_KEY_SMILEY_REPLACEMENT = JitsiApplication.getResString(R.string.pref_key_smiley_replacement);
    /*
    ...
    // Notifications
    static private final String P_KEY_POPUP_HANDLER = JitsiApplication.getResString(R.string.pref_key_popup_handler);

    // Call section
    static private final String P_KEY_NORMALIZE_PNUMBER = JitsiApplication.getResString(R.string.pref_key_normalize_pnumber);
    static private final String P_KEY_ACCEPT_ALPHA_PNUMBERS = JitsiApplication.getResString(R.string.pref_key_accept_alpha_pnumbers);

    // Audio settings
    static private final String P_KEY_AUDIO_ECHO_CANCEL = JitsiApplication.getResString(R.string.pref_key_audio_echo_cancel);
    static private final String P_KEY_AUDIO_AGC = JitsiApplication.getResString(R.string.pref_key_audio_agc);
    static private final String P_KEY_AUDIO_DENOISE = JitsiApplication.getResString(R.string.pref_key_audio_denoise);

    // Video settings
    static private final String P_KEY_VIDEO_CAMERA = JitsiApplication.getResString(R.string.pref_key_video_camera);
    // Hardware encoding(API16)
    static private final String P_KEY_VIDEO_HW_ENCODE = JitsiApplication.getResString(R.string.pref_key_video_hw_encode);
    // Direct surface encoding(hw encoding required and API18)
    static private final String P_KEY_VIDEO_ENC_DIRECT_SURFACE = JitsiApplication.getResString(R.string.pref_key_video_surface_encode);
    // Hardware decoding(API16)
    static private final String P_KEY_VIDEO_HW_DECODE = JitsiApplication.getResString(R.string.pref_key_video_hw_decode);
    // Video resolutions
    static private final String P_KEY_VIDEO_RES = JitsiApplication.getResString(R.string.pref_key_video_resolution);
    // Video advanced settings
    static private final String P_KEY_VIDEO_LIMIT_FPS = JitsiApplication.getResString(R.string.pref_key_video_limit_fps);
    static private final String P_KEY_VIDEO_TARGET_FPS = JitsiApplication.getResString(R.string.pref_key_video_target_fps);
    static private final String P_KEY_VIDEO_MAX_BANDWIDTH = JitsiApplication.getResString(R.string.pref_key_video_max_bandwidth);
    static private final String P_KEY_VIDEO_BITRATE = JitsiApplication.getResString(R.string.pref_key_video_bitrate);
    ...
    public static class SettingsFragment extends OSGiPreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        private void initMessagesPreferences() {
            PreferenceUtil.setCheckboxVal(getPreferenceScreen(), P_KEY_LOG_CHAT_HISTORY, AndroidGUIActivator.getMessageHistoryService().isHistoryLoggingEnabled());
            ...
            PreferenceUtil.setCheckboxVal(getPreferenceScreen(), P_KEY_TYPING_NOTIFICATIONS, ConfigurationUtils.isSendTypingNotifications());

            ConfigurationService cfg = AndroidGUIActivator.getConfigurationService();

            PreferenceUtil.setCheckboxVal(getPreferenceScreen(), P_KEY_SMILEY_REPLACEMENT, cfg.getBoolean(ReplacementProperty.getPropertyName("SMILEY"), true));
        ...
        private void updateHistorySizeSummary() {
            EditTextPreference historySizePref = (EditTextPreference) findPreference(P_KEY_HISTORY_SIZE);
            historySizePref.setSummary(getString(R.string.service_gui_settings_CHAT_HISTORY_SUMMARY, ConfigurationUtils.getChatHistorySize()));
        }
        ...
        private void initNotificationPreferences() {
            ...
            String configuredHandler = (String) AndroidGUIActivator.getConfigurationService().getProperty("systray.POPUP_HANDLER");
            int idx = 1;
        ...
        private void initCallPreferences() {
            PreferenceUtil.setCheckboxVal(getPreferenceScreen(), P_KEY_NORMALIZE_PNUMBER, ConfigurationUtils.isNormalizePhoneNumber());

            PreferenceUtil.setCheckboxVal(getPreferenceScreen(), P_KEY_ACCEPT_ALPHA_PNUMBERS, ConfigurationUtils.acceptPhoneNumberWithAlphaChars());
        ...
        private void initVideoPreferences() {
            ...
            if (currentCamera != null) cameraList.setValue(currentCamera.getLocator().toString());
            ...
            Dimension autoResDim = new Dimension(DeviceConfiguration.DEFAULT_VIDEO_WIDTH, DeviceConfiguration.DEFAULT_VIDEO_HEIGHT);
            ...
            targetFpsPref.setText(targetFps != DeviceConfiguration.DEFAULT_VIDEO_FRAMERATE ? Integer.toString(targetFps) : defaultFpsStr);
            ...
            EditTextPreference maxBandwidthPref = (EditTextPreference) findPreference(P_KEY_VIDEO_MAX_BANDWIDTH);
            ...
            summaryMapper.includePreference(maxBandwidthPref, DeviceConfiguration.DEFAULT_VIDEO_RTP_PACING_THRESHOLD + "");
            summaryMapper.includePreference(bitratePref, DeviceConfiguration.DEFAULT_VIDEO_BITRATE + "");
        }
        ...
        private static Dimension resoultionForStr(String resStr) {
            ...
            for (Dimension resolution : resolutions) {
                if (resToStr(resolution).equals(resStr)) return resolution;
            }
        ...
        private void updateHwCodecStatus(AndroidCamera selectedCamera) {
            ...
            boolean enableMediaCodecs = selectedCamera != null && DeviceSystem.LOCATOR_PROTOCOL_ANDROIDCAMERA.equals(selectedCamera.getCameraProtocol());
        ...
        private PopupMessageHandler getHandlerForClassName(String clazz) {
            ...
            for (ServiceReference sRef : handlerRefs) {
                PopupMessageHandler handler = (PopupMessageHandler) bc.getService(sRef);
                if (handler.getClass().getName().equals(clazz)) return handler;
            }
            return null;
        }
        ...
        public void onSharedPreferenceChanged(SharedPreferences shPreferences, String key) {
            if (key.equals(P_KEY_LOG_CHAT_HISTORY)) {
                ...
                mhs.setHistoryLoggingEnabled(shPreferences.getBoolean(P_KEY_LOG_CHAT_HISTORY, mhs.isHistoryLoggingEnabled()));
            }
                ...
                String intStr = shPreferences.getString(P_KEY_HISTORY_SIZE, "" + ConfigurationUtils.getChatHistorySize());
                ConfigurationUtils.setChatHistorySize(Integer.parseInt(intStr));
                updateHistorySizeSummary();
            }
            else if (key.equals(P_KEY_TYPING_NOTIFICATIONS)) {
                ConfigurationUtils.setSendTypingNotifications(shPreferences.getBoolean(P_KEY_TYPING_NOTIFICATIONS, ConfigurationUtils.isSendTypingNotifications()));
            }
            else if (key.equals(P_KEY_SMILEY_REPLACEMENT)) {
                AndroidGUIActivator.getConfigurationService().setProperty(ReplacementProperty.getPropertyName("SMILEY"), shPreferences.getBoolean(P_KEY_SMILEY_REPLACEMENT, true));
            }
            /* Chat alerter is not implemented on Android
            else if(key.equals(P_KEY_CHAT_ALERTS))
            {
                ...
                ConfigurationUtils.setAcceptPhoneNumberWithAlphaChars(shPreferences.getBoolean(P_KEY_ACCEPT_ALPHA_PNUMBERS, true));
            }
                ...
                ((EditTextPreference) findPreference(P_KEY_VIDEO_MAX_BANDWIDTH)).setText(deviceConfig.getVideoRTPPacingThreshold() + "");
            }
            else if (key.equals(P_KEY_VIDEO_BITRATE)) {
                String bitrateStr = shPreferences.getString(P_KEY_VIDEO_BITRATE, "");
                int bitrate = !bitrateStr.isEmpty() ? Integer.parseInt(bitrateStr) : DeviceConfiguration.DEFAULT_VIDEO_BITRATE;
                if (bitrate < 1) {
                    bitrate = 1;
                }
                deviceConfig.setVideoBitrate(bitrate);

                ((EditTextPreference) findPreference(P_KEY_VIDEO_BITRATE)).setText(bitrate + "");
            }
        }
    }
}
