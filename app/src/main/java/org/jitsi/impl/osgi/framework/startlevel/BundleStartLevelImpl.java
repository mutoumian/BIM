...
public class BundleStartLevelImpl implements BundleStartLevel {
    ...
    public int getStartLevel() {
        ...
        if (startLevel == 0) {
            FrameworkStartLevel frameworkStartLevel = getBundle().getFramework().adapt(FrameworkStartLevel.class);

            if (frameworkStartLevel == null) startLevel = 1;
            else startLevel = frameworkStartLevel.getInitialBundleStartLevel();
        }
        return startLevel;
    }
