...
public class EventListenerList {
    ...
    public synchronized <T extends EventListener> boolean add(Bundle bundle, Class<T> clazz, T listener) {
        ...
        if (index == -1) return elements.add(new Element<T>(bundle, clazz, listener));
        else return false;
    }
    ...
    public synchronized <T extends EventListener> T[] getListeners(Class<T> clazz) {
        ...
        for (Element<?> element : elements)
            if (element.clazz == clazz) eventListeners[count++] = element.listener;

        @SuppressWarnings("unchecked") T[] listeners = (T[]) Array.newInstance(clazz, count);
    ...
    private synchronized <T extends EventListener> int indexOf(Bundle bundle, Class<T> clazz, T listener) {
        for (int index = 0, count = elements.size(); index < count; index++) {
            ...
            if (element.bundle.equals(bundle) && (element.clazz == clazz) && (element.listener == listener))
                return index;
        }
        return -1;
    }
    ...
    public synchronized boolean removeAll(Bundle bundle) {
        ...
        for (int index = 0, count = elements.size(); index < count; ) {
            if (elements.get(index).bundle.equals(bundle) && (elements.remove(index) != null))
                changed = true;
            else index++;
        }
