...
public class UpdateServiceImpl implements UpdateService {
    ...
    public void checkForUpdates(boolean notifyAboutNewestVersion) {
        ...
        ResourceManagementService R = ServiceUtils.getService(UpdateActivator.bundleContext, ResourceManagementService.class);
        ...
        if (!isLatest && downloadLink != null) {
            ...
            if (previousDownloads.size() > 0) {
                ...
                if (lastJobStatus == DownloadManager.STATUS_SUCCESSFUL) {
                    ...
                    AndroidUtils.showAlertDialog(JitsiApplication.getGlobalContext(), R.getI18NString("plugin.updatechecker.DIALOG_IN_PROGRESS_TITLE"), R.getI18NString("plugin.updatechecker.DIALOG_IN_PROGRESS"));
                    return;
                }
            }

            AndroidUtils.showAlertConfirmDialog(JitsiApplication.getGlobalContext(), R.getI18NString("plugin.updatechecker.DIALOG_TITLE"), R.getI18NString("plugin.updatechecker.DIALOG_MESSAGE", new String[]{R.getI18NString("app.name")}), R.getI18NString("plugin.updatechecker.BUTTON_DOWNLOAD"), new DialogActivity.DialogListener() {
                        @Override
            ...
            AndroidUtils.showAlertDialog(JitsiApplication.getGlobalContext(), R.getI18NString("plugin.updatechecker.DIALOG_NOUPDATE_TITLE"), R.getI18NString("plugin.updatechecker.DIALOG_NOUPDATE"));
        }
    }
    ...
    private int checkDownloadStatus(long id) {
        ...
        try {
            ...
            else return cursor.getInt(cursor.getColumnIndex(DownloadManager.COLUMN_STATUS));
        }
        finally {
            cursor.close();
        }
    ...
    private SharedPreferences getStore() {
        if (store == null) {
            store = JitsiApplication.getGlobalContext().getSharedPreferences("store", Context.MODE_PRIVATE);
        }
        return store;
    }
    ...
    private List<Long> getOldDownloads() {
        ...
        for (String idStr : idStrs) {
            try {
                if (!idStr.isEmpty()) apkIds.add(Long.parseLong(idStr));
            }
