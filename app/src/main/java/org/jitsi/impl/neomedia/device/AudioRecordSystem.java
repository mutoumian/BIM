...
public class AudioRecordSystem extends AudioSystem {
    ...
    protected void doInitialize() throws Exception {
        ...
        for (int i = 0; i < Constants.AUDIO_SAMPLE_RATES.length; i++) {
            ...
            formats.add(new AudioFormat(AudioFormat.LINEAR, sampleRate, 16 /* sampleSizeInBits */, 1 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray));
        }

        CaptureDeviceInfo2 captureDevice = new CaptureDeviceInfo2("android.media.AudioRecordCapture", new MediaLocator(LOCATOR_PROTOCOL + ":"), formats.toArray(new Format[formats.size()]), null, null, null);
        List<CaptureDeviceInfo2> captureDevices = new ArrayList<CaptureDeviceInfo2>(1);

        captureDevices.add(captureDevice);
        setCaptureDevices(captureDevices);

        CaptureDeviceInfo2 playbackDevice = new CaptureDeviceInfo2("android.media.AudioRecordPlayback", new MediaLocator(LOCATOR_PROTOCOL + ":playback"), formats.toArray(new Format[formats.size()]), null, null, null);
        CaptureDeviceInfo2 notificationDevice = new CaptureDeviceInfo2("android.media.AudioRecordNotification", new MediaLocator(LOCATOR_PROTOCOL + ":notification"), formats.toArray(new Format[formats.size()]), null, null, null);
