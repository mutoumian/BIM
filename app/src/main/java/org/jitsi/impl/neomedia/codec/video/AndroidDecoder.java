...
public class AndroidDecoder extends AndroidCodec {
     ...
     * Input formats list.
     */
    private static final VideoFormat[] INPUT_FORMATS = new VideoFormat[]{new VideoFormat(Constants.VP8), new VideoFormat(Constants.H263P), new VideoFormat(Constants.H264), new ParameterizedVideoFormat(Constants.H264, JNIEncoder.PACKETIZATION_MODE_FMTP, "0")};
    ...
    public AndroidDecoder() {
        ...
        if (isHwDecodingEnabled()) inputFormats = INPUT_FORMATS;
        else inputFormats = EMPTY_FORMATS;
    ...
    protected Format[] getMatchingOutputFormats(Format inputFormat) {
        if (!(inputFormat instanceof VideoFormat)) return EMPTY_FORMATS;

        VideoFormat inputVideoFormat = (VideoFormat) inputFormat;
        if (useSurface()) {
            return new VideoFormat[]{
                    //new SurfaceFormat(inputVideoFormat.getSize()),
                    new VideoFormat(Constants.ANDROID_SURFACE, inputVideoFormat.getSize(), Format.NOT_SPECIFIED, Surface.class, Format.NOT_SPECIFIED)};
        }
        else {
    ...
    public Format setOutputFormat(Format format) {
        ...
        if (format instanceof YUVFormat) {
            ...
            outputFormat = new VideoFormat(videoFormat.getEncoding(), outputSize, videoFormat.getMaxDataLength(), videoFormat.getDataType(), videoFormat.getFrameRate());
        }
