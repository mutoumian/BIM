...
public class DataSource extends AbstractPushBufferCaptureDevice {
    ...
    private FileDescriptor createLocalSocket(MediaRecorder mediaRecorder) throws IOException {
        ...
        synchronized (DataSource.class) {
            if (DataSource.localServerSocket == null) {
                ...
                localServerSocketThread.setName(DataSource.localServerSocket.getLocalSocketAddress().getName());
        ...
        if (localSocketKey != null) localSocketKey = null;
    ...
    private void discard(InputStream inputStream, long byteCount) throws IOException {
        while (byteCount-- > 0)
            if (-1 == inputStream.read()) throw new IOException(ENDOFSTREAM_IOEXCEPTION_MESSAGE);
    }
     ...
     */
    @Override
    protected synchronized void doStart() throws IOException {
        if (this.mediaRecorder == null) {
            ...
            try {
                ...
                else mediaRecorder.setOutputFile(OUTPUT_FILE);
            ...
            if (exception != null) {
                ...
                if (exception instanceof ThreadDeath) throw (ThreadDeath) exception;
                else {
    ...
    private void configure(Camera camera, MediaRecorder mediaRecorder, VideoFormat videoFormat) {
        ...
        if ((videoSize != null) && (videoSize.height > 0) && (videoSize.width > 0)) {
            logger.warn("Will attempt to capture from " + getLocator() + " in " + videoSize.width + "x" + videoSize.height + ". May not be supported.");
    ...
    private H264Parameters obtainParameters(Camera camera, MediaRecorder mediaRecorder) throws IOException {
        final String sampleFile = JitsiApplication.getGlobalContext().getCacheDir().getPath() + "/jitsi-test.mpeg4";
    ...
    protected synchronized void doStop() throws IOException {
        ...
        try {
            ...
            if (mediaRecorderStopState != null) {
                ...
                if (logger.isDebugEnabled() && (mediaRecorderStopState[0] != MEDIA_RECORDER_STOPPED)) {
    ...
    private void localSocketAccepted(LocalSocket localSocket, InputStream inputStream) {
        ...
        try {
            ...
            if (maxLocalSocketKeySize > 0) {
                ...
                if (localSocketKey == null) throw new IOException(UNEXPECTED_IOEXCEPTION_MESSAGE);
            }
            else throw new IOException(UNEXPECTED_IOEXCEPTION_MESSAGE);
            ...
            if (DUMP_FILE != null) dump = new FileOutputStream(DUMP_FILE + "." + localSocketKey);
            ...
            while (true) {
                ...
                if (type == FILE_TYPE_BOX_TYPE) {
                     ...
                     */
                    size = 4 /* size */ + 4 /* type */ + 4 /* major_brand */ + 4 /* minor_version */ + 4 /* compatible_brands[0] == "isom" */ + 4 /* compatible_brands[1] == "3gp4" */;
                    ...
                    if (size == 1) {
                        size = readUnsignedInt64(inputStream) /* largesize */ - 8;
                    }
                    if (size == 0) {
    ...
    private static String readLine(InputStream inputStream, int maxSize) throws IOException {
        int size = 0;
        int b;
    ...
    private void readNAL(String localSocketKey, byte[] bytes, int nalLength) throws IOException {
        synchronized (this) {
            ...
            synchronized (nalSyncRoot) {
                if ((nal == null) || (nal.length < nalLength)) nal = new byte[nalLength];
                this.nalLength = 0;
    ...
    private void readNAL(String localSocketKey, InputStream inputStream, int nalLength) throws IOException {
        byte[] delayed = null;
        ...
        synchronized (this) {
            ...
            synchronized (nalSyncRoot) {
                ...
                while (remainingToRead > 0) {
                    ...
                    if (-1 == read) throw new IOException(ENDOFSTREAM_IOEXCEPTION_MESSAGE);
                    else {
    ...
    public static long readUnsignedInt(InputStream inputStream, int byteCount) throws IOException {
        long value = 0;

        for (int i = byteCount - 1; i >= 0; i--) {
            int b = inputStream.read();

            if (-1 == b) throw new IOException(ENDOFSTREAM_IOEXCEPTION_MESSAGE);
            else {
                if ((i == 7) && ((b & 0x80) != 0))
        ...
        Thread mediaRecorderStop = new Thread("MediaRecorder.stop") {
            ...
            public void run() {
                try {
                    ...
                    if (trace) logger.trace("Stopping MediaRecorder in " + this);
                    mediaRecorder.stop();
                    if (trace) {
                    ...
                    mediaRecorder.release();
                    if (trace) logger.trace("Released MediaRecorder in " + this);
                }
                catch (Throwable t) {
    ...
    private void writeNAL() {
        ...
        synchronized (getStreamSyncRoot()) {
            ...
            stream = ((streams != null) && (streams.length != 0)) ? (MediaRecorderStream) streams[0] : null;
        }
    ...
    private static class MediaRecorderStream extends AbstractPushBufferStream {
        ...
        public void read(Buffer buffer) throws IOException {
            ...
            int byteLength = DePacketizer.NAL_PREFIX.length + FFmpeg.FF_INPUT_BUFFER_PADDING_SIZE;
            byte[] bytes = null;
            int flags = 0;
            long timeStamp = 0;
            ...
            synchronized (dataSource.nalSyncRoot) {
                ...
                if (nalLength > 0) {
                    ...
                    if (data instanceof byte[]) {
                        bytes = (byte[]) data;
                        if (bytes.length < byteLength) bytes = null;
                    }
                    else bytes = null;
                    if (bytes == null) {
                    ...
                    System.arraycopy(dataSource.nal, 0, bytes, DePacketizer.NAL_PREFIX.length, nalLength);
            ...
            buffer.setOffset(0);
            if (bytes == null) buffer.setLength(0);
            else {
                System.arraycopy(DePacketizer.NAL_PREFIX, 0, bytes, 0, DePacketizer.NAL_PREFIX.length);
