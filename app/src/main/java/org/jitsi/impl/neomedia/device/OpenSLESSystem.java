...
public class OpenSLESSystem extends AudioSystem {
    ...
    protected void doInitialize() throws Exception {
        ...
        int[] audioInputCapabilities = queryAudioInputCapabilities(SL_DEFAULTDEVICEID_AUDIOINPUT, supportedSampleRates, SUPPORTED_SAMPLE_SIZES_IN_BITS, SUPPORTED_CHANNELS);
        ...
        if (audioInputCapabilities != null) {
            ...
            while (true) {
                ...
                formats.add(new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, channels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray));
            }
        }
         ...
         */
        if (formats.isEmpty()) {
            formats.add(new AudioFormat(AudioFormat.LINEAR, 44100 /* sampleRate */, 16 /* sampleSizeInBits */, 1 /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray));
        }

        if (!formats.isEmpty()) {
            CaptureDeviceInfo2 captureDevice = new CaptureDeviceInfo2("OpenSL ES", new MediaLocator(LOCATOR_PROTOCOL + ":"), formats.toArray(new Format[formats.size()]), null, null, null);
    ...
    private static native int[] queryAudioInputCapabilities(long deviceID, double[] sampleRates, int[] sampleSizesInBits, int[] channels);
     ...
     */
    public InputStream getAudioInputStream(String url) throws IOException {
