...
public class MediaRecorderSystem extends DeviceSystem {
    ...
    protected void doInitialize() throws Exception {
        ...
        for (int cameraId = 0; cameraId < cameraCount; cameraId++) {
            if (cameraInfo == null) cameraInfo = new Camera.CameraInfo();
            ...
            List<Dimension> sizes = new ArrayList<Dimension>(CameraUtils.PREFERRED_SIZES.length);
            ...
            try {
                ...
                if (supportedSizes == null) {
                    ...
                    if (logger.isDebugEnabled() && (supportedSizes != null)) {
                        logger.debug("Preview sizes supported by " + locator + ": " + CameraUtils.cameraSizesToString(supportedSizes));
                    }
                }
                else if (logger.isDebugEnabled()) {
                    logger.debug("Video sizes supported by " + locator + ": " + CameraUtils.cameraSizesToString(supportedSizes));
                }
                if (supportedSizes != null) {
                    for (Dimension preferredSize : CameraUtils.PREFERRED_SIZES) {
                        for (Camera.Size supportedSize : supportedSizes) {
                            if ((preferredSize.height == supportedSize.height) && (preferredSize.width == supportedSize.width)) {
            ...
            for (int i = 0; i < count; i++) {
                formats[i] = new ParameterizedVideoFormat(Constants.H264, sizes.get(i), Format.NOT_SPECIFIED /* maxDataLength */, Format.byteArray, Format.NOT_SPECIFIED /* frameRate */, ParameterizedVideoFormat.toMap(JNIEncoder.PACKETIZATION_MODE_FMTP, "1"));
            }

            // Create display name
            Resources res = JitsiApplication.getAppResources();
            String name = facing.equals(CAMERA_FACING_FRONT) ? res.getString(R.string.service_gui_settings_FRONT_CAMERA) : res.getString(R.string.service_gui_settings_BACK_CAMERA);
            name += " (MediaRecorder)";
            ...
            else captureDevices.add(device);
        }
