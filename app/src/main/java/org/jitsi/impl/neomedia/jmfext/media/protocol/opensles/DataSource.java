...
public class DataSource extends AbstractPullBufferCaptureDevice {
    ...
    private static native long connect(String encoding, double sampleRate, int sampleSizeInBits, int channels, int endian, int signed, Class<?> dataType) throws IOException;
    ...
    private static native int read(long ptr, Object data, int offset, int length) throws IOException;
    ...
    private static native void start(long ptr) throws IOException;

    private static native void stop(long ptr) throws IOException;
    ...
    private static class OpenSLESStream extends AbstractPullBufferStream {
        ...
        public synchronized void connect() throws IOException {
            if (ptr == 0) {
                ...
                ptr = DataSource.connect(format.getEncoding(), sampleRate, sampleSizeInBits, channels, format.getEndian(), format.getSigned(), format.getDataType());
                if (ptr == 0) throw new IOException();
                else {
                    length = (int) (20 /* milliseconds */ * (sampleRate / 1000) * channels * (sampleSizeInBits / 8));
        ...
        public void read(Buffer buffer) throws IOException {
            ...
            while (read < 1) {
                synchronized (this) {
                    if (started) {
                        if (ptr == 0) throw new IOException("ptr");
                        else read = DataSource.read(ptr, data, offset, length);
                    }
                    else break;
                }
            }
