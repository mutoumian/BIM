...
abstract class AndroidCodec extends AbstractCodec2 {
     ...
     */
    protected abstract void configureMediaCodec(MediaCodec codec, String codecType) throws ResourceUnavailableException;
    ...
    protected void doOpen() throws ResourceUnavailableException {
        ...
        logger.info("Opened " + codecType + " " + getStrName() + " for name: " + codecInfo.getName() + " use surface ? " + useSurface());
    }
        ...
        if (mediaCodecOutputIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
            ...
            if (!isEncoder) {
                ...
                if (!useSurface() && pixelFormat != requestedFormat) {
                    throw new RuntimeException("MediaCodec returned different color format: " + pixelFormat + "(requested " + requestedFormat + ", try using the Surface");
                }
            }
            // Video size should be known at this point
            onSizeDiscovered(new Dimension(outFormat.getInteger(MediaFormat.KEY_WIDTH), outFormat.getInteger(MediaFormat.KEY_HEIGHT)));
        }
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("Reading output: " + info.offset + ":" + info.size + " flag: " + info.flags);
            }
            ...
            try {
                if (!isEncoder && useSurface()) {
                    ...
                    byte[] out = AbstractCodec2.validateByteArraySize(outputBuffer, info.size, false);
        ...
        if (isEncoder && useSurface()) {
            ...
            if (mediaCodecInputIndex >= 0) {
                ...
                if (codecInput.capacity() < inputLen) {
                    throw new RuntimeException("Input buffer too small: " + codecInput.capacity() + " < " + inputLen);
                }
