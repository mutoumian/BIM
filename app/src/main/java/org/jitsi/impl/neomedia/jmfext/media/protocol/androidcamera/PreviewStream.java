...
public class PreviewStream extends CameraStreamBase implements Camera.PreviewCallback {
    ...
    protected void onInitPreview() throws IOException {
        ...
        logger.info(prevSize.width + "x" + prevSize.height + " using buffers of size: " + bufferSize + " for image format: 0x" +
    ...
    static void YV12toYUV420Planar(final byte[] input, final byte[] output, final int width, final int height) {
        if (width % 16 != 0) throw new IllegalArgumentException("Unsupported width: " + width);
        ...
        if (uvStridePadding == 0) {
            ...
            for (int y = 0; y < height / 2; y++) {
                System.arraycopy(input, src, output, I420uvSize + dst, I420uvStride); // Cr (V)
                System.arraycopy(input, uvSize + src, output, dst, I420uvStride); // Cb (U)
                src += uvStride;
                dst += I420uvStride;
            }
        }
