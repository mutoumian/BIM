...
public class LogUploadServiceImpl implements LogUploadService {
     ...
     */
    private static final String storagePath = Environment.getExternalStorageDirectory().getPath() + "/jitsi-logs/";
    ...
    public void sendLogs(String[] destinations, String subject, String title) {
        try {
            ...
            try {
                logcatFile = LoggingUtilsActivatorEx.getFileAccessService().getPrivatePersistentFile(new File("log", "jitsi-current-logcat.txt").toString(), FileCategory.LOG);
