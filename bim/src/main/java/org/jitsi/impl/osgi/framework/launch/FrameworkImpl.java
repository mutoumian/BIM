...
public class FrameworkImpl extends BundleImpl implements Framework {
    ...
    public <A> A adapt(Class<A> type) {
        ...
        @SuppressWarnings("unchecked") A a = (A) adapt;
        ...
        if (id == 0) return this;
            ...
            synchronized (this.bundles) {
                for (BundleImpl bundle : this.bundles)
                    if (bundle.getBundleId() == id) return bundle;
            }
            return null;
        }
    }
    ...
    private List<BundleImpl> getBundlesByStartLevel(int startLevel) {
        ...
        synchronized (this.bundles) {
            for (BundleImpl bundle : this.bundles) {
                ...
                if ((bundleStartLevel != null) && (bundleStartLevel.getStartLevel() == startLevel))
                    bundles.add(bundle);
            }
        }
        return bundles;
    ...
    public Collection<ServiceReference> getServiceReferences(BundleImpl origin, Class<?> clazz, String className, Filter filter, boolean checkAssignable) throws InvalidSyntaxException {
        Filter classNameFilter = FrameworkUtil.createFilter('(' + Constants.OBJECTCLASS + '=' + ((className == null) ? '*' : className) + ')');
        List<ServiceReference> serviceReferences = new LinkedList<ServiceReference>();
        ...
        synchronized (serviceRegistrations) {
            for (ServiceRegistrationImpl serviceRegistration : serviceRegistrations) {
                ...
                if (classNameFilter.match(serviceReference) && ((filter == null) || (filter.match(serviceReference)))) {
                    ServiceReference serviceReferenceS = serviceRegistration.getReference(clazz);

                    if (serviceReferenceS != null) serviceReferences.add(serviceReferenceS);
                }
            }
        }
    ...
    public Bundle installBundle(BundleImpl origin, String location, InputStream input) throws BundleException {
        if (location == null) throw new BundleException("location");
        ...
        synchronized (bundles) {
            ...
            if (bundle == null) {
                bundle = new BundleImpl(getFramework(), getNextBundleId(), location);
                bundles.add(bundle);
    ...
    public ServiceRegistration registerService(BundleImpl origin, Class clazz, String[] classNames, Object service, Dictionary<String, ?> properties) {
        ...
        else if (service == null) throw new IllegalArgumentException("service");
        else {
            ...
            if (!ServiceFactory.class.isAssignableFrom(serviceClass)) {
                ...
                for (String className : classNames) {
                    ...
                    if (illegalArgumentException) {
                        ...
                        if (cause != null) iae.initCause(cause);
                        throw iae;
                    }
                }
            }
        ...
        fireServiceEvent(ServiceEvent.REGISTERED, serviceRegistration.getReference());
    ...
    public void start(int options) throws BundleException {
        ...
        if (state == STARTING) {
            ...
            if (configuration != null) {
                String s = configuration.get(Constants.FRAMEWORK_BEGINNING_STARTLEVEL);
            ...
            synchronized (listener) {
                ...
                while (frameworkStartLevel.getStartLevel() < startLevel) try {
                    listener.wait();
                }
        ...
        new Thread(getClass().getName() + ".stop") {
            ...
            public void run() {
                ...
                synchronized (listener) {
                    ...
                    while (frameworkStartLevel.getStartLevel() != 0) try {
                        listener.wait();
                    }
                    catch (InterruptedException ie) {
                        interrupted = true;
    ...
    public void unregisterService(BundleImpl origin, ServiceRegistration<?> serviceRegistration) {
        ...
        else throw new IllegalStateException("serviceRegistrations");
    }
