...
public class AsyncExecutor<T extends Runnable> {
    ...
    public AsyncExecutor(long keepAliveTime, TimeUnit unit) {
        if (keepAliveTime < 0) throw new IllegalArgumentException("keepAliveTime");
    ...
    private synchronized boolean contains(T command) {
        for (CommandFuture<T> commandFuture : queue)
            if (commandFuture.command == command) return true;
        return false;
    }
        ...
        while (true) {
            ...
            synchronized (this) {
                if (shutdownNow) return;
                     ...
                     */
                    if (idleTime == -1) idleTime = System.currentTimeMillis();
                    else if ((System.currentTimeMillis() - idleTime) > keepAliveTime) return;
            ...
            try {
                ...
                else uncaughtException(command, t);
            }
            finally {
    ...
    public synchronized Future<?> submit(T command) {
        ...
        if (contains(command)) throw new RejectedExecutionException("contains");
    ...
    private synchronized void startThreadOrNotifyAll() {
        if ((thread == null) && (!shutdown && !shutdownNow) && !queue.isEmpty()) {
    ...
    private static class CommandFuture<T extends Runnable> implements Future<Object> {
        ...
        public synchronized Object get(long timeout, TimeUnit unit) throws ExecutionException, InterruptedException, TimeoutException {
            ...
            while (true) {
                ...
                else if (timeoutException) throw new TimeoutException();
                else {
                    wait(timeout);
        ...
        synchronized void setDone(Object done) {
            if (done instanceof Boolean) this.done = (Boolean) done;
            else if (done instanceof Throwable) exception = (Throwable) done;
            else throw new IllegalArgumentException("done");
