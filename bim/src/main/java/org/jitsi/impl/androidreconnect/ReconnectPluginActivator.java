...
public class ReconnectPluginActivator implements BundleActivator, ServiceListener, NetworkConfigurationChangeListener, RegistrationStateChangeListener {
     ...
     */
    private final HashSet<ProtocolProviderService> autoReconnEnabledProviders = new HashSet<ProtocolProviderService>();
                ...
                try {
                    ...
                    if (pp.getRegistrationState().equals(RegistrationState.UNREGISTERING) || pp.getRegistrationState().equals(RegistrationState.UNREGISTERED) || pp.getRegistrationState().equals(RegistrationState.CONNECTION_FAILED)) {
                        if (reconnect) {
                            ...
                            if (logger.isInfoEnabled()) logger.info("Reconnect " +
                                    pp.getAccountID().getDisplayName() + " after " + task.delay + " ms.");
    ...
    private void traceCurrentPPState() {
        ...
        logger.trace("autoReconnEnabledProviders: " + autoReconnEnabledProviders);
    ...
    private void notify(String title, String i18nKey, String[] params, Object tag) {
        ...
        extras.put(NotificationData.POPUP_MESSAGE_HANDLER_TAG_EXTRA, tag);
        ...
        synchronized (this) {
            try {
                ...
                if (evt.getNewState().equals(RegistrationState.CONNECTION_FAILED)) {
                    ...
                    autoReconnEnabledProviders.add(pp);
            ...
            if (thread == null || !Thread.currentThread().equals(thread)) {
                ...
                try {
                    if (logger.isInfoEnabled())
                        logger.info("Start reconnecting " + provider.getAccountID().getDisplayName());
    ...
    private long getInitialReconnectDelay() {
        ...
        return (long) (RECONNECT_DELAY_MIN + Math.random() * RECONNECT_DELAY_MAX) * 1000;
        //}
        //else
        //{
