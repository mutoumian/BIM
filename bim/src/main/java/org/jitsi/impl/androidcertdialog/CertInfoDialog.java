...
public class CertInfoDialog extends OSGiDialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (certDialog == null) throw new RuntimeException("No dialog model found for: " + request);
        ...
        View content = getActivity().getLayoutInflater().inflate(R.layout.cert_info, null);
        ...
        if (cert instanceof X509Certificate) {
            ...
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_ISSUED_ON), dateFormat.format(x509.getNotBefore()));
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_EXPIRES_ON), dateFormat.format(x509.getNotAfter()));

            // Fingerprints
            try {
                appendTxtRow(table, R.string.service_gui_CERT_INFO_FINGERPRINTS);
            ...
            // Serial number
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_SER_NUM), x509.getSerialNumber().toString());
            // Version
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_VER), String.valueOf(x509.getVersion()));
            // Signature algorithm
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_SIGN_ALG), String.valueOf(x509.getSigAlgName()));
            ...
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_ALG), algorithm);

            if (algorithm.equals("RSA")) {
                // Public key
                RSAPublicKey key = (RSAPublicKey) x509.getPublicKey();

                String keyStr = getString(R.string.service_gui_CERT_INFO_KEY_BYTES_PRINT, String.valueOf(key.getModulus().toByteArray().length - 1), key.getModulus().toString(16));

                appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_PUB_KEY), keyStr);

                appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_EXP), String.valueOf(key.getPublicExponent()));

                String keySizeStr = getString(R.string.service_gui_CERT_INFO_KEY_BITS_PRINT, String.valueOf(key.getModulus().bitLength()));
                appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_KEY_SIZE), keySizeStr);
            }
            else if (algorithm.equals("DSA")) {
                DSAPublicKey key = (DSAPublicKey) x509.getPublicKey();
                appendTxtRow(table, "Y:", key.getY().toString(16));
            }

            // Signature
            String signatureStr = getString(R.string.service_gui_CERT_INFO_KEY_BYTES_PRINT, String.valueOf(x509.getSignature().length), getHex(x509.getSignature()));
            appendTxtRow(table, getString(R.string.service_gui_CERT_INFO_SIGN), signatureStr);
        }
        else {
        ...
        CompoundButton alwaysTrustBtn = (CompoundButton) content.findViewById(R.id.alwaysTrust);
