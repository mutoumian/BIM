...
public class AndroidEncoder extends AndroidCodec {
     ...
     */
    private static final VideoFormat[] SUPPORTED_OUTPUT_FORMATS = new VideoFormat[]{new VideoFormat(Constants.VP8), new VideoFormat(Constants.H263P), new ParameterizedVideoFormat(Constants.H264, JNIEncoder.PACKETIZATION_MODE_FMTP, "0"), new ParameterizedVideoFormat(Constants.H264, JNIEncoder.PACKETIZATION_MODE_FMTP, "1")};
    ...
    public static boolean isDirectSurfaceEnabled() {
        ...
        return isHwEncodingEnabled() && supported && LibJitsi.getConfigurationService().getBoolean(DIRECT_SURFACE_ENCODE_PROPERTY, supported);
    }
    ...
    public AndroidEncoder() {
        ...
        if (useInputSurface) {
            inputFormats = new VideoFormat[]{new VideoFormat(Constants.ANDROID_SURFACE, null, Format.NOT_SPECIFIED, Surface.class, Format.NOT_SPECIFIED)};
        }
        else {
    ...
    protected Format[] getMatchingOutputFormats(Format inputFormat) {
        if (!(inputFormat instanceof VideoFormat) || !isHwEncodingEnabled()) return EMPTY_FORMATS;

        VideoFormat inputVideoFormat = (VideoFormat) inputFormat;
        Dimension size = inputVideoFormat.getSize();
        float frameRate = inputVideoFormat.getFrameRate();

        return new VideoFormat[]{new VideoFormat(Constants.VP8, size,
                        /* maxDataLength */ Format.NOT_SPECIFIED, Format.byteArray, frameRate), new VideoFormat(Constants.H263P, size, Format.NOT_SPECIFIED, Format.byteArray, frameRate), new ParameterizedVideoFormat(Constants.H264, size, Format.NOT_SPECIFIED, Format.byteArray, frameRate, ParameterizedVideoFormat.toMap(JNIEncoder.PACKETIZATION_MODE_FMTP, "0")), new ParameterizedVideoFormat(Constants.H264, size, Format.NOT_SPECIFIED, Format.byteArray, frameRate, ParameterizedVideoFormat.toMap(JNIEncoder.PACKETIZATION_MODE_FMTP, "1"))};
    }
    ...
    public Format setOutputFormat(Format format) {
        if (!(format instanceof VideoFormat) || (matches(format, getMatchingOutputFormats(inputFormat)) == null))
            return null;
        ...
        if (inputFormat != null) size = ((VideoFormat) inputFormat).getSize();
    ...
    @Override
    protected void configureMediaCodec(MediaCodec codec, String codecType) throws ResourceUnavailableException {
        if (outputFormat == null) throw new ResourceUnavailableException("Output format not set");
        ...
        // Select color format
        int colorFormat = useInputSurface ? MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface : MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar;
