...
public class H264Parameters implements Serializable {
     ...
     * @throws IOException if we failed to parse and extract values.
     */
     ...
     */
    private void parse(String path, RandomAccessFile file) throws IOException {
        ...
        while (true) {
            ...
            if (name.equals("moov") || name.equals("trak") || name.equals("mdia") || name.equals("minf") || name.equals("stbl") || name.equals("stsd")) {
     ...
     */
    private void parseStsdBox(RandomAccessFile file) throws IOException {
     ...
     */
    private void discard(RandomAccessFile file, long count) throws IOException {
     ...
     */
    private long readUnsignedInt(RandomAccessFile file, int byteCount) throws IOException {
        long value = 0;
        ...
        for (int i = byteCount - 1; i >= 0; i--) {
            ...
            if (-1 == b) throw new IOException("End of stream");
                ...
                if ((i == 7) && ((b & 0x80) != 0)) throw new IOException("Integer overflow");
                value |= ((b & 0xFFL) << (8 * i));
            }
        }
        return value;
     ...
     */
    private long readUnsignedInt32(RandomAccessFile file) throws IOException {
     ...
     * @throws IOException if EOF or other error occurs.
     */
    private long readUnsignedInt64(RandomAccessFile file) throws IOException {
    ...
    static H264Parameters getStoredParameters(VideoFormat formatUsed) {
        SharedPreferences config = JitsiApplication.getGlobalContext().getSharedPreferences(STORE_ID, Context.MODE_PRIVATE);

        // Checks if the video size matches
        String storedSizeStr = config.getString(VIDEO_SIZE_STORE_ID, null);
        if (!formatUsed.getSize().toString().equals(storedSizeStr)) return null;
    ...
    static void storeParameters(H264Parameters params, VideoFormat formatUsed) {
        ...
        config.edit().putString(STORE_ID, storeString).putString(VIDEO_SIZE_STORE_ID, formatUsed.getSize().toString()).commit();
    }
}