...
public class AndroidMediaCodec extends AbstractCodec2 {
     ...
     */
    private static final long DEQUEUE_INPUT_BUFFER_TIMEOUT = /* second */ 1000000 / /* frames per second */ 30;
     ...
     */
    private static final Logger logger = Logger.getLogger(AndroidMediaCodec.class);
    ...
    static {
        ...
        PIX_FMTS_TO_MEDIA_CODEC_COLOR_FORMATS = new int[]{FFmpeg.PIX_FMT_NV12, MediaCodecInfo.CodecCapabilities.COLOR_TI_FormatYUV420PackedSemiPlanar};
        ...
        for (int codecIndex = 0, codecCount = MediaCodecList.getCodecCount(); codecIndex < codecCount; codecIndex++) {
            ...
            for (String supportedType : supportedTypes) {
                ...
                if (aSupportedFormat == null) continue;
                ...
                if ((colorFormats == null) || (colorFormats.length == 0)) continue;

                if (bSupportedFormats != null) bSupportedFormats.clear();
                for (int colorFormat : colorFormats) {
                    Format bSupportedFormat = getFmjFormatFromMediaCodecColorFormat(colorFormat);

                    if (bSupportedFormat == null) continue;
                ...
                if (loggerIsDebugEnabled) {
                    ...
                    if ((profileLevels != null) && (profileLevels.length != 0)) {
                        ...
                        for (int i = 0; i < profileLevels.length; i++) {
                            ...
                            s.append("profile= ").append(profileLevel.profile).append(", level= ").append(profileLevel.level);
                        }
                        s.append("];");
                    }
    ...
    private static MediaCodecInfo.CodecCapabilities getCapabilitiesForType(MediaCodecInfo codecInfo, String type) {
        ...
        try {
            ...
            capabilities = null;
            logger.warn("Invocation failed for supported/mime type: " + type, iae);
        }
        return capabilities;
    }
    ...
    protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
        ...
        if ((inputFormat != null) && (inputFormat != this.inputFormat) && !inputFormat.equals(inputFormat) && (null == setInputFormat(inputFormat))) {
         ...
         */
        if (!(outputFormat instanceof AVFrameFormat)) return BUFFER_PROCESSED_FAILED;
        ...
        if (MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED == mediaCodecOutputIndex) {
             ...
             * feed input into it.
             */
            if ((processed == BUFFER_PROCESSED_FAILED) || (outputLength > 0)) return processed;
        }
        ...
        if (0 <= mediaCodecInputIndex) {
            ...
            try {
                ...
                if (mediaCodecInputLength == fmjLength) processed &= ~INPUT_BUFFER_NOT_CONSUMED;
                else {
                ...
                mediaCodec.queueInputBuffer(mediaCodecInputIndex, mediaCodecInputOffset, mediaCodecInputLength,
    ...
    private void maybeConfigureAndStart() {
        ...
        if (mediaCodec != null) {
            ...
            if (!matchesMediaCodecType(typeFormat, type) || !matchesMediaCodecColorFormat(colorFormatFormat, colorFormat)) {
                doClose();
            }
        }
        ...
        if ((this.colorFormat != OMX_COLOR_FormatUnused) && (this.type != null)) {
            ...
            if (mediaCodec != null) {
                ...
                if (avFrame == null) avFrame = new AVFrame();
            }
        }
         ...
         */
        if (this.mediaCodec == null) throw new IllegalStateException("mediaCodec");
    }
