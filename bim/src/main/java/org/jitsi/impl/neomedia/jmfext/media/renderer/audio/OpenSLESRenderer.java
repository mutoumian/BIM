...
public class OpenSLESRenderer extends AbstractAudioRenderer<AudioSystem> {
    ...
    static {
        ...
        for (int i = 0; i < supportedInputSampleRateCount; i++) {
            SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, supportedInputSampleRates[i], 16 /* sampleSizeInBits */, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
        }
    }
    ...
    public synchronized void open() throws ResourceUnavailableException {
        if (ptr == 0) {
            ...
            ptr = open(inputFormat.getEncoding(), inputFormat.getSampleRate(), inputFormat.getSampleSizeInBits(), channels, inputFormat.getEndian(), inputFormat.getSigned(), inputFormat.getDataType());
            if (ptr == 0) throw new ResourceUnavailableException();

            setThreadPriority = true;
        }
    }

    private static native long open(String encoding, double sampleRate, int sampleSizeInBits, int channels, int endian, int signed, Class<?> dataType) throws ResourceUnavailableException;
