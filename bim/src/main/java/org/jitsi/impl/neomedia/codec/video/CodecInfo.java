...
public abstract class CodecInfo {
    ...
    static {
        ...
        for (int codecIndex = 0, codecCount = MediaCodecList.getCodecCount(); codecIndex < codecCount; codecIndex++) {
            ...
            logger.info("Discovered codec: " + codecInfo.getName() + "/" + Arrays.toString(codecInfo.getSupportedTypes()));
    ...
    public static CodecInfo getCodecForType(String mimeType, boolean isEncoder) {
        for (CodecInfo codec : codecs) {
            if (!codec.isBanned() && codec.mediaType.equals(mimeType) && codec.codecInfo.isEncoder() == isEncoder) {
                return codec;
            }
        }
        return null;
    ...
    public static CodecInfo getCodecInfo(MediaCodecInfo codecInfo) {
        ...
        for (String type : types) {
            try {
                ...
                logger.error("Error initializing codec info: " + codecInfo.getName() + ", type: " + type, e);
            }
        }
        return null;
    }
    ...
    static class H263CodecInfo extends CodecInfo {
        private final Profile[] PROFILES = new Profile[]{
                ...
                new Profile("Baseline", 0x01), new Profile("H320Coding", 0x02), new Profile("BackwardCompatible", 0x04), new Profile("ISWV2", 0x08), new Profile("ISWV3", 0x10), new Profile("HighCompression", 0x20), new Profile("Internet", 0x40), new Profile("Interlace", 0x80), new Profile("HighLatency", 0x100)};

        private final Level[] LEVELS = new Level[]{
                // from OMX_VIDEO_H263LEVELTYPE
                new Level("Level10", 0x01), new Level("Level20", 0x02), new Level("Level30", 0x04), new Level("Level40", 0x08), new Level("Level45", 0x10), new Level("Level50", 0x20), new Level("Level60", 0x40), new Level("Level70", 0x80)};
    ...
    static class VP8CodecInfo extends CodecInfo {
        ...
        private final Level[] LEVELS = new Level[]{
                ...
                new Level("Version0", 0x01), new Level("Version1", 0x02), new Level("Version2", 0x04), new Level("Version3", 0x08)};
