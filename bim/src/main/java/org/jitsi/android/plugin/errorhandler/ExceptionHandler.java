...
public class ExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        logger.fatal("Uncaught exception occurred, killing the process...", ex);

        // Save logcat for more information.
        File logFile;
        try {
            logFile = ExceptionHandlerActivator.getFileAccessService().getPrivatePersistentFile(new File("log", "jitsi-crash-logcat.txt").toString(), FileCategory.LOG);
    ...
    private static SharedPreferences getStorage() {
        return JitsiApplication.getGlobalContext().getSharedPreferences("crash", Context.MODE_PRIVATE);
    }
