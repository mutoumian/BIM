...
public class AndroidSecurityAuthority implements SecurityAuthority {
    ...
    public UserCredentials obtainCredentials(String realm, UserCredentials defaultValues, int reasonCode) {
        if (reasonCode == SecurityAuthority.AUTHENTICATION_REQUIRED || reasonCode == WRONG_PASSWORD || reasonCode == WRONG_USERNAME)
     ...
     * if none could be obtained.
     */
    public UserCredentials obtainCredentials(String realm, final UserCredentials defaultValues) {
        ...
        // Login argument
        args.putString(CredentialsFragment.ARG_LOGIN, defaultValues.getUserName());
        ...
        if (password != null) {
            args.putString(CredentialsFragment.ARG_PASSWORD, defaultValues.getPasswordAsString());
        }

        // Persistent password argument
        args.putBoolean(CredentialsFragment.ARG_STORE_PASSWORD, defaultValues.isPasswordPersistent());

        // If OSGi has not started wait for
        // the <tt>LauncherActivity</tt> to finish
        if (AndroidGUIActivator.bundleContext == null) {
            final Object currentActivityMonitor = JitsiApplication.getCurrentActivityMonitor();
            synchronized (currentActivityMonitor) {
                while (true) {
                    Activity currentActivity = JitsiApplication.getCurrentActivity();
                    if (currentActivity != null && !(currentActivity instanceof LauncherActivity)) {
                        break;
                    }
                    try {
                        logger.warn("Enter password dialog waiting for any Activity");
        ...
        DialogActivity.showCustomDialog(JitsiApplication.getGlobalContext(), resources.getI18NString("service.gui.LOGIN_FAILED"), CredentialsFragment.class.getName(), args, resources.getI18NString("sign_in"), new DialogActivity.DialogListener() {
                    public boolean onConfirmClicked(DialogActivity dialog) {
                        ...
                        boolean storePassword = ViewUtil.
                                isCompoundChecked(dialogContent, R.id.store_password);
