...
public class ChatSecuritySettings extends OSGiActivity {
    ...
    // Preference keys
    static private final String P_KEY_OTR_ENABLE = JitsiApplication.getResString(org.jitsi.R.string.pref_key_otr_enable);
    static private final String P_KEY_OTR_AUTO = JitsiApplication.getResString(org.jitsi.R.string.pref_key_otr_auto);
    static private final String P_KEY_OTR_REQUIRE = JitsiApplication.getResString(R.string.pref_key_otr_require);
    ...
    public static class SettingsFragment extends OSGiPreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        public void onStart() {
            ...
            PreferenceUtil.setCheckboxVal(screen, P_KEY_OTR_ENABLE, otrPolicy.getEnableManual());

            PreferenceUtil.setCheckboxVal(screen, P_KEY_OTR_AUTO, otrPolicy.getEnableAlways());

            PreferenceUtil.setCheckboxVal(screen, P_KEY_OTR_REQUIRE, otrPolicy.getRequireEncryption());
        ...
        public void onSharedPreferenceChanged(SharedPreferences shPreferences, String key) {
            ...
            if (key.equals(P_KEY_OTR_ENABLE)) {
                otrPolicy.setEnableManual(shPreferences.getBoolean(P_KEY_OTR_ENABLE, otrPolicy.getEnableManual()));
            }
            else if (key.equals(P_KEY_OTR_AUTO)) {
                boolean isAutoInit = shPreferences.getBoolean(P_KEY_OTR_AUTO, otrPolicy.getEnableAlways());

                otrPolicy.setEnableAlways(isAutoInit);
                OtrActivator.configService.setProperty(OtrActivator.AUTO_INIT_OTR_PROP, Boolean.toString(isAutoInit));
            }
            else if (key.equals(P_KEY_OTR_REQUIRE)) {
                boolean isRequired = shPreferences.getBoolean(P_KEY_OTR_REQUIRE, otrPolicy.getRequireEncryption());
