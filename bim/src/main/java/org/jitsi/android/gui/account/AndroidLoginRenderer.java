...
public class AndroidLoginRenderer implements LoginRenderer {
     ...
     * The logger
     */
    private final static Logger logger = Logger.getLogger(AndroidLoginRenderer.class);
     ...
     */
    private EventListenerList<PresenceStatus> globalStatusListeners = new EventListenerList<PresenceStatus>();
     ...
     */
    public void protocolProviderConnectionFailed(final ProtocolProviderService protocolProvider, final LoginManager loginManagerCallback) {
        AccountID accountID = protocolProvider.getAccountID();

        AndroidUtils.showAlertConfirmDialog(JitsiApplication.getGlobalContext(), JitsiApplication.getResString(R.string.service_gui_ERROR), JitsiApplication.getResString(R.string.service_gui_CONNECTION_FAILED_MSG, accountID.getUserID(), accountID.getService()), JitsiApplication.getResString(R.string.service_gui_RETRY), new DialogActivity.DialogListener() {
    ...
    public void updateJitsiIconNotification() {
        ...
        if (notificationID == -1) {
            logger.debug("Not displaying status notification because" + " there's no global notification icon available.");
            return;
        }
    ...
    synchronized public Drawable getLocalStatusDrawable() {
        ...
        if (statusImage != localStatusRaw) {
            ...
            localStatusDrawable = localStatusRaw != null ? AndroidImageUtil.drawableFromBytes(statusImage) : null;
        }
