...
public class Account implements ProviderPresenceStatusListener, RegistrationStateChangeListener, ServiceListener, AvatarListener {
     ...
     */
    private EventListenerList<AccountEvent> listeners = new EventListenerList<AccountEvent>();
    ...
    private void setProtocolProvider(ProtocolProviderService protocolProvider) {
        if (this.protocolProvider != null && protocolProvider != null) {
            ...
            logger.warn("This account have already registered provider " + "- will update");
            // Unregister old
            setProtocolProvider(null);
            // Register new
            setProtocolProvider(protocolProvider);
        ...
        if (protocolProvider != null) {
            ...
            OperationSetPresence presenceOpSet = this.protocolProvider.getOperationSet(OperationSetPresence.class);
            if (presenceOpSet != null) {
                presenceOpSet.removeProviderPresenceStatusListener(this);
            }

            OperationSetAvatar avatarOpSet = this.protocolProvider.getOperationSet(OperationSetAvatar.class);
            if (avatarOpSet != null) {
    ...
    public Drawable getStatusIcon() {
        ...
        return AccountUtil.getDefaultPresenceIcon(activityContext, accountID.getProtocolName());
    }
    ...
    public Drawable getAvatarIcon() {
        if (avatarIcon == null) {
            ...
            try {
                ...
                logger.error("Error retrieving avatar: " + exc.getMessage());
            }
