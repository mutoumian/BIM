...
public class PresenceStatusHandler implements ServiceListener, RegistrationStateChangeListener {
    ...
    public void start(BundleContext bundleContext) {
        ...
        ServiceReference<ProtocolProviderService>[] pps = ServiceUtils.getServiceReferences(bundleContext, ProtocolProviderService.class);
    ...
    private void updateStatus(ProtocolProviderService protocolProvider) {
        ...
        if (!protocolProvider.isRegistered()) presenceStatus = offlineStatus;
        else {
        ...
        if (protocolProvider.isRegistered() && !presence.getPresenceStatus().equals(presenceStatus)) {
            AndroidGUIActivator.getGlobalStatusService().publishStatus(protocolProvider, presenceStatus, false);
        }
    }
