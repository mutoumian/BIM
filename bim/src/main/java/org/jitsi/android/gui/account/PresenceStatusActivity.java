...
public class PresenceStatusActivity extends OSGiActivity implements EventListener<AccountEvent>, AdapterView.OnItemSelectedListener {
     ...
     */
    static final private Logger logger = Logger.getLogger(PresenceStatusActivity.class);
    ...
    protected void onCreate(android.os.Bundle savedInstanceState) {
        ...
        this.account = new Account(accountID, AndroidGUIActivator.bundleContext, this);
    ...
    private void initPresenceStatus() {
        ...
        if (accountPresence == null) {
            Toast t = Toast.makeText(this, getString(R.string.service_gui_PRESENCE_NOT_SUPPORTED, account.getAccountName()), Toast.LENGTH_LONG);
            t.show();
        ...
        // Account properties
        String titlePattern = getResources().getString(R.string.service_gui_PRESENCE_EDIT_TITLE);

        String title = MessageFormat.format(titlePattern, account.getAccountName());
        ...
        statusSpinner.setSelection(statusAdapter.getPositionForItem(accountPresence.getPresenceStatus()));

        statusSpinner.setOnItemSelectedListener(this);

        // Status edit
        EditText statusEdit = (EditText) findViewById(R.id.presenceStatusMessageEdit);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.remove) {
            RemoveAccountDialog.create(this, account.getAccountID(), new RemoveAccountDialog.OnAccountRemovedListener() {
                        @Override
            ...
            Intent preferences = AccountPreferencesActivity.getIntent(this, account.getAccountID());
    ...
    public void onAvatarClicked(View avatarView) {
        if (account.getAvatarOpSet() == null) {
            logger.warn("Avatar operation set is not supported by " + account.getAccountName());
            return;
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != SELECT_IMAGE || resultCode != RESULT_OK) return;

        final OperationSetAvatar avatarOpSet = account.getAvatarOpSet();
        if (avatarOpSet == null) {
            logger.warn("No avatar operation set found for " + account.getAccountName());
        ...
        try {
            ...
            Bitmap bmp = AndroidImageUtil.scaledBitmapFromContentUri(this, imageUri, PREFERRED_SIZE, PREFERRED_SIZE);
    ...
    private void showAvatarChangeError() {
        AndroidUtils.showAlertDialog(this, getString(R.string.service_gui_ERROR), getString(R.string.service_gui_AVATAR_SET_ERROR, account.getAccountName()));
    }
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    GlobalStatusService globalStatus = ServiceUtils.getService(AndroidGUIActivator.bundleContext, GlobalStatusService.class);
                    globalStatus.publishStatus(account.getProtocolProvider(), status, true);
    ...
    private void commitStatusChanges() {
        ...
        Spinner statusSpinner = (Spinner) findViewById(R.id.presenceStatusSpinner);
        EditText statusMessageEdit = (EditText) findViewById(R.id.presenceStatusMessageEdit);
     ...
     */
    class StatusListAdapter extends CollectionAdapter<PresenceStatus> {
        /**
        ...
        protected View getView(boolean isDropDown, PresenceStatus item, ViewGroup parent, LayoutInflater inflater) {
            ...
            // Retrieve views
            View statusItemView = inflater.inflate(R.layout.presence_status_row, parent, false);
        ...
        int getPositionForItem(PresenceStatus status) {
            for (int i = 0; i < getCount(); i++) {
                ...
                if (other.equals(status)) return i;
            }
            return -1;
        }
    }
