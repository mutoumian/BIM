...
public class TypingNotificationHandler {
     ...
     */
    public static void handleTypingNotificationReceived(TypingNotificationEvent evt, ChatFragment chatFragment) {
        typingTimer.cancel();
        ...
        MetaContact metaContact = chatFragment.getChatSession().getMetaContact();

        /**
         * If the given event doesn't concern the chat fragment meta contact
         * we have nothing more to do here.
         */
        if (!metaContact.containsContact(evt.getSourceContact())) return;
        ...
        if (typingState == OperationSetTypingNotifications.STATE_TYPING || typingState == OperationSetTypingNotifications.STATE_PAUSED) {
            new UpdateTypingTask(chatFragment, typingState).execute();

            typingTimer.schedule(new TypingTimerTask(chatFragment), 5000);
        }
        else {
    ...
    private static class UpdateTypingTask extends AsyncTask<Void, Void, Void> {
        ...
        public UpdateTypingTask(ChatFragment chatFragment, int typingState) {
            this.chatFragment = chatFragment;
    ...
    private static class RemoveTypingTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            ...
            chatFragment.setTypingState(OperationSetTypingNotifications.STATE_STOPPED);
        }
    }
}
