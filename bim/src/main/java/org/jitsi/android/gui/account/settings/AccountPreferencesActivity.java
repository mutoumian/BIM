...
public class AccountPreferencesActivity extends OSGiActivity {
     ...
     * The logger
     */
    private static final Logger logger = Logger.getLogger(AccountPreferencesActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            this.preferencesFragment = createPreferencesFragment(userUniqueID, protocolName);

            // Display the fragment as the main content.
            getFragmentManager().beginTransaction().replace(android.R.id.content, preferencesFragment).commit();
        }
        else {
            preferencesFragment = (AccountPreferenceFragment) getFragmentManager().findFragmentById(android.R.id.content);
        }
    }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (commitThread != null) return true;
    ...
    public static Intent getIntent(Context ctx, AccountID accountID) {
        ...
        intent.putExtra(AccountPreferencesActivity.EXTRA_USER_ID, accountID.getAccountUniqueID());
