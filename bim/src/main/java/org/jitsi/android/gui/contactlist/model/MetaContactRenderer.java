...
public class MetaContactRenderer implements UIContactRenderer {
    ...
    private static String getDisplayDetails(MetaContact metaContact) {
        ...
        while (protoContacts.hasNext()) {
            ...
            if (authOpSet != null && authOpSet.getSubscriptionStatus(protoContact) != null && !authOpSet.getSubscriptionStatus(protoContact).equals(OperationSetExtendedAuthorizations.SubscriptionStatus.Subscribed)) {
                ...
                if (status.equals(OperationSetExtendedAuthorizations.SubscriptionStatus.SubscriptionPending))
                    subscriptionDetails = AndroidGUIActivator.getResourcesService().getI18NString("service.gui.WAITING_AUTHORIZATION");
                else if (status.equals(OperationSetExtendedAuthorizations.SubscriptionStatus.NotSubscribed))
                    subscriptionDetails = AndroidGUIActivator.getResourcesService().getI18NString("service.gui.NOT_AUTHORIZED");
            }
    ...
    public static Drawable getStatusDrawable(MetaContact metaContact) {
        ...
        if (statusImage != null) return AndroidImageUtil.drawableFromBytes(statusImage);
