...
public class ChatPagerAdapter extends FragmentStatePagerAdapter implements ChatListener {
    ...
    public int getChatIdx(String sessionId) {
        ...
        for (int i = 0; i < chats.size(); i++) {
            if (getChatId(i).equals(sessionId)) return i;
        }
    ...
    public void setPrimaryItem(ViewGroup container, int position, Object object) {
        ...
        if (newPrimary != primaryItem) {
            if (primaryItem != null) primaryItem.setVisibleToUser(false);
            if (newPrimary != null) newPrimary.setVisibleToUser(true);
        }
