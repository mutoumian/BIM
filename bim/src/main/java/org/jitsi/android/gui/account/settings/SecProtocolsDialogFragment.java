...
public class SecProtocolsDialogFragment extends DialogFragment {
     ...
     * The logger
     */
    private static final Logger logger = Logger.getLogger(SecProtocolsDialogFragment.class);
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            this.protocolsAdapter = new ProtocolsAdapter(savedInstanceState.getStringArray(STATE_ENCRYPTIONS), (Map<String, Boolean>) savedInstanceState.get(STATE_STATUS_MAP));
        }
    ...
    public void commit(SecurityAccountRegistration securityReg) {
        ...
        for (int i = 0; i < protocolsAdapter.encryptions.length; i++) {
            protocols.put(protocolsAdapter.encryptions[i], new Integer(i));
        }
    ...
    class ProtocolsAdapter extends BaseAdapter implements TouchInterceptor.DropListener {
        ...
        ProtocolsAdapter(final Map<String, Integer> encryptions, final Map<String, Boolean> statusMap) {
            ...
            for (String enc : encryptions.keySet()) {
                if (!statusMap.containsKey(enc)) statusMap.put(enc, false);
            }
