...
class CtrlNotificationThread {
    ...
    private void notificationLoop() {
        while (run) {
            ...
            notification.contentView.setInt(R.id.mute_button, "setBackgroundResource", isMute ? R.mipmap.status_btn_on : R.mipmap.status_btn_off);

            boolean isOnHold = CallManager.isLocallyOnHold(call);
            notification.contentView.setInt(R.id.hold_button, "setBackgroundResource", isOnHold ? R.mipmap.status_btn_on : R.mipmap.status_btn_off);
