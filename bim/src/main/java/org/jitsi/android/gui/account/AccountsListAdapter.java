...
public class AccountsListAdapter extends CollectionAdapter<Account> implements EventListener<AccountEvent>, ServiceListener {
     ...
     */
    public AccountsListAdapter(Activity parent, int listRowResourceID, int dropDownRowResourceID, Collection<AccountID> accounts, boolean filterDisabledAccounts) {
        super(parent);
    ...
    public void serviceChanged(ServiceEvent event) {
        ...
        if (event.getType() == ServiceEvent.REGISTERED) {
            ...
            if (acc == null) {
                addAccount(new Account(protocolProvider.getAccountID(), bundleContext, getParentActivity().getBaseContext()));
            }
            else {
    ...
    protected View getView(boolean isDropDown, Account account, ViewGroup parent, LayoutInflater inflater) {
        ...
        TextView accountName = (TextView) statusItem.findViewById(R.id.accountName);
        ...
        if (accountName != null) accountName.setText(account.getAccountName());
        ...
        if (accountStatus != null) accountStatus.setText(account.getStatusName());
    ...
    private Account findAccountID(AccountID account) {
        for (int i = 0; i < getCount(); i++) {
            ...
            if (acc.getAccountID().equals(account)) return acc;
        }
        return null;
    }
    ...
    public void onChangeEvent(AccountEvent accountEvent) {
        if (logger.isTraceEnabled()) {
            logger.trace("Received accountEvent update " + accountEvent.getSource().getAccountName() + " " + accountEvent.toString(), new Throwable());
        }
        doRefreshList();
    }
}
