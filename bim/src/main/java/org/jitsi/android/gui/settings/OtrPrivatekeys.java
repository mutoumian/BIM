...
public class OtrPrivatekeys extends OSGiActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.accountsAdapter = new PrivateKeyListAdapter(accounts);

        accountsKeysList.setAdapter(accountsAdapter);

        accountsKeysList.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
                    @Override
    ...
    private void showGenerateKeyAlert(int position) {
        ...
        int getResStrId = OtrActivator.scOtrKeyManager.getLocalFingerprint(account) != null ? R.string.plugin_otr_REGENERATE_QUESTION : R.string.plugin_otr_GENERATE_QUESTION;

        b.setTitle(getString(R.string.plugin_otr_GENERATE_DLG_TITLE)).setMessage(getString(getResStrId, account.getDisplayName())).setPositiveButton(R.string.service_gui_YES, new DialogInterface.OnClickListener() {
                    @Override
     ...
     * <tt>AccountID</tt>s.
     */
    class PrivateKeyListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            View row = getLayoutInflater().inflate(R.layout.otr_privkey_list_row, parent, false);

            AccountID acc = (AccountID) getItem(position);

            ViewUtil.setTextViewValue(row, R.id.accountName, acc.getDisplayName());

            String fingerprint = OtrActivator.scOtrKeyManager.getLocalFingerprint(acc);

            String fingerprintStr;

            if (fingerprint == null || fingerprint.length() < 1) {
                fingerprintStr = OtrActivator.resourceService.getI18NString("plugin.otr.configform.NO_KEY_PRESENT");
            }
            else {
