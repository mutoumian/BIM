...
public class MetaContactListAdapter extends BaseContactListAdapter implements MetaContactListListener, ContactPresenceStatusListener, UIGroupRenderer {
     ...
     */
    public void dispose() {
        if (contactListService == null) return;
    ...
    private TreeSet<MetaContact> getOriginalCList(int groupIndex) {
        if (groupIndex >= 0 && groupIndex < originalContacts.size()) {
            ...
            logger.warn("Get original contact list for idx: " + groupIndex + ", list size: " + originalContacts.size(), new Throwable());
            return null;
        }
    }
    ...
    private TreeSet<MetaContact> getContactList(int groupIndex) {
        if (groupIndex >= 0 && groupIndex < contacts.size()) {
            ...
            logger.warn("Get contact list for idx: " + groupIndex + ", list size: " + contacts.size(), new Throwable());
            return null;
        }
    }
    ...
    private void addGroup(final MetaContactGroup group) {
        if (originalGroups.indexOf(group) < 0) {
            ...
            TreeSet<MetaContact> originalChildren = new TreeSet<MetaContact>();

            originalContacts.add(originalChildren);
        }

        if (isMatching(group, currentQuery) && groups.indexOf(group) < 0) {
            groups.add(group);

            TreeSet<MetaContact> children = new TreeSet<MetaContact>();
     ...
     */
    private void addContact(final MetaContactGroup metaGroup, final MetaContact metaContact) {
        ...
        boolean isMatchingQuery = isMatching(metaContact, currentQuery);
        ...
        TreeSet<MetaContact> origContactList = getOriginalCList(groupIndex);
        ...
        if (isMatchingQuery && contactList != null && getChildIndex(contactList, metaContact) < 0) {
     ...
     */
    private void removeContact(final MetaContactGroup metaGroup, final MetaContact metaContact) {
        ...
        if (origGroupIndex >= 0) {
            ...
            if (origContactList.size() <= 0) removeGroup(metaGroup);
        }
        ...
        if (groupIndex >= 0) {
            ...
            if (contactList.size() <= 0) removeGroup(metaGroup);
        }
    }
    ...
    private void updateStatus(final MetaContact metaContact) {
        ...
        if (contactIndex >= 0) updateStatus(groupIndex, contactIndex, metaContact);
    }
        ...
        uiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (oldGroupIdx < 0 || newGroupIdx < 0) {
                    logger.error("Move group error - original list, srcGroupIdx: " + oldGroupIdx + ", dstGroupIdx: " + newGroupIdx);
                }
                else {
                ...
                if (oldGroupIdx < 0 || newGroupIdx < 0) {
                    logger.error("Move group error, srcGroupIdx: " + oldGroupIdx + ", dstGroupIdx: " + newGroupIdx);
                }
                else {
            ...
            public void run() {
                ...
                if (origGroupIndex >= 0) {
                    ...
                    if (contactList != null) {
                        ...
                        originalContacts.add(origGroupIndex, new TreeSet<MetaContact>(contactList));
                    }
                }
    ...
    public Object getChild(int groupPosition, int childPosition) {
        ...
        Iterator<MetaContact> contactList = getContactList(groupPosition).iterator();
        int i = 0;
    ...
    private int getChildIndex(TreeSet<MetaContact> contactList, MetaContact metaContact) {
        Iterator<MetaContact> contactListIter = contactList.iterator();

        int i = 0;
        while (contactListIter.hasNext()) {
            if (metaContact.equals(contactListIter.next())) return i;
            i++;
        }
    ...
    public void filterData(final String query) {
        ...
        if (query.isEmpty()) {
            ...
            for (MetaContactGroup metaGroup : originalGroups) {
                ...
                TreeSet<MetaContact> filteredList = new TreeSet<MetaContact>();

                for (MetaContact metaContact : contactList) {
                    if (isMatching(metaContact, query)) filteredList.add(metaContact);
                }
    ...
    private boolean isMatching(MetaContact metaContact, String query) {
        ...
        if (queryPattern.matcher(metaContact.getDisplayName()).find()) return true;
            ...
            while (contacts.hasNext()) {
                ...
                if (queryPattern.matcher(contact.getDisplayName()).find() || queryPattern.matcher(contact.getAddress()).find()) {
                    return true;
                }
            }
        }
    ...
    public void contactPresenceStatusChanged(final ContactPresenceStatusChangeEvent event) {
        uiHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (metaContact != null) updateStatus(metaContact);
            }
        });
    }
    ...
    public static boolean isContactSelected(MetaContact metaContact) {
        return ChatSessionManager.getCurrentChatId() != null && ChatSessionManager.getActiveChat(metaContact) != null && ChatSessionManager.getCurrentChatId().equals(ChatSessionManager.getActiveChat(metaContact).getChatId());
    }
    ...
    public String getDisplayName(Object groupImpl) {
        ...
        if (metaGroup.equals(contactListService.getRoot()))
            return JitsiApplication.getResString(R.string.service_gui_CONTACTS);
        else return metaGroup.getGroupName();
    }
}
